...
// Kräver env: GOOGLE_CLIENT_ID, GOOGLE_CLIENT_SECRET, GOOGLE_REDIRECT_URI
...
export default async function handler(req, res) {
  ...
  if (error) {
    return redirect(res, `/testa-faktura?oauth_error=${encodeURIComponent(error)}&provider=gmail`);
  }
  if (!code) {
    return redirect(res, '/testa-faktura?oauth_error=no_code&provider=gmail');
  }

  const clientId     = process.env.GOOGLE_CLIENT_ID;
  const clientSecret = process.env.GOOGLE_CLIENT_SECRET;
  const redirectUri  = process.env.GOOGLE_REDIRECT_URI ?? `${BASE_URL}/api/auth/gmail-callback`;

  if (!clientId || !clientSecret) {
    return redirect(res, '/testa-faktura?oauth_pending=gmail');
  }
  ...
  try {
    ...
    if (!tokenRes.ok) {
      ...
      return redirect(res, '/testa-faktura?oauth_error=token_exchange&provider=gmail');
    }
    ...
    console.error('[gmail-callback] Token fetch error:', err.message);
    return redirect(res, '/testa-faktura?oauth_error=network&provider=gmail');
  }
  ...
  if (db) {
    try {
      ...
      console.error('[gmail-callback] DB error:', err.message);
    }
  }

  // ── Search Gmail for invoice PDFs ─────────────────────────────────────────
  let invoiceCount = 0;
  try {
    const messages = await gmailSearch(
      accessToken,
      '(subject:faktura OR subject:invoice OR subject:räkning) has:attachment newer_than:180d',
      20
    );
    invoiceCount = messages.length;
  } catch (err) {
    console.error('[gmail-callback] Gmail search error:', err.message);
  }
  ...
  if (userEmail) {
    ...
    const countText = invoiceCount > 0
      ...
      : 'Vi har kopplat er Gmail-inkorg.';
...
<tr><td style="padding:24px 36px;">
  <p style="font-size:12px;color:#8A9E98;text-align:center;margin:0;line-height:1.6;">
    ...
    try {
      await resend.emails.send({
        from: FROM,
        to: userEmail,
        subject: 'Arvo Intelligence aktiverat — Gmail kopplat',
        html,
      });
    } catch (err) {
      console.error('[gmail-callback] Confirmation email failed:', err.message);
    }
  }
  ...
  try {
    await resend.emails.send({
      from: FROM,
      to: INTERNAL,
      subject: `[Gmail kopplat] ${userEmail ?? 'okänd'} — ${invoiceCount} fakturor hittade`,
  ...
  // ── Redirect to success ───────────────────────────────────────────────────
  const params = new URLSearchParams({ intelligence_connected: 'gmail' });
