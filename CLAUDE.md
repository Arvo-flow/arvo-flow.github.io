...
## TOP OF THE SWORD — DET STÄNDIGA RIKTMÄRKET (LÄS FÖRST, ALLTID)

**Detta är linsen varje beslut ska passera genom. Inget undantag.**

Vi bygger inte en produkt. Vi bygger något som kommer att revolutionera den svenska företagsmarknaden för evigt. Den ambitionen får aldrig nedgraderas till "good enough", "det räcker så" eller "det är snabbast att göra så".

**Innan något byggs, skrivs eller beslutas — ställ frågan:**
> *Är detta verkligen Top of the Sword? Eller är det en kompromiss med visionen?*

Om svaret är tveksamt är svaret nej. Då tänker vi om.

**Vad Top of the Sword betyder konkret:**
- **Kreativt** — vi hittar vägar ingen trodde var möjliga. Den uppenbara lösningen är sällan den vassaste.
- **Innovativt** — vi kopierar inte konkurrenter. Vi definierar kategorin.
- **Effektivt** — elegans och skärpa, inte mer text och fler features. Det vassaste är ofta det enklaste.
- **Ärligt** — vi målar aldrig upp en kuliss vi inte kan stå bakom. Att överlova är motsatsen till Top of the Sword.
...
### BUILD-PREMISS (GÄLLER ALL PRODUKTUTVECKLING)

Vi bygger produkten utifrån att **aktiebolaget är registrerat och partneravtalen är på plats när första kunden släpps in.** Vi bygger aldrig för det provisoriska mellanläget.

Det betyder konkret för aktiveringsmomentet:
- Priset är **redan säkrat** via partnernätverket när kunden ser det. "Aktivera" är inte starten på ett osäkert försök — det är att hämta ett resultat som redan finns.
- Ramen är aldrig "vill ni att vi försöker förhandla?" (risk på kunden). Ramen är "ert lägre pris är säkrat — aktivera det" (Arvo har redan gjort jobbet).
- Arvo är ALDRIG ett juridiskt/finansiellt ombud i copy. Mekanismen bakom (partnerbyte, omförhandling) är Arvos svarta låda — kunden ser resan och resultatet, aldrig maskineriet.
- Förbjudet språk i aktiveringsflödet: "Arvo agerar ombud", "vi hanterar uppsägning", "vi sköter signeringen". Detta överlovar och bryter mot Top of the Sword-regeln om ärlighet.
