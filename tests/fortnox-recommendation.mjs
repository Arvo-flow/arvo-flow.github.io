...
describe('Fortnox · paket-igenkänning', () => {
  test('känner igen "Fortnox Paket Stor"', () => {
    const p = detectSaasFinancePaket(inv('Fortnox Paket Stor', 710));
    assert.equal(p.name, 'Stor'); assert.equal(p.monthly, 710); assert.equal(p.billedMonthly, 710);
  });
  test('känner igen Mini/Liten/Mellan', () => {
    assert.equal(detectSaasFinancePaket(inv('Fortnox Mini')).name, 'Mini');
    assert.equal(detectSaasFinancePaket(inv('Paket Liten')).name, 'Liten');
    assert.equal(detectSaasFinancePaket(inv('Fortnox paket Mellan')).name, 'Mellan');
  });
  test('rad UTAN fortnox/paket-ord ger ingen falsk träff (t.ex. "Stor skärm")', () => {
    assert.equal(detectSaasFinancePaket(inv('Stor skärm 27 tum', 2000)), null);
  });
  test('okänd rad → null', () => {
    assert.equal(detectSaasFinancePaket(inv('Microsoft 365 Business', 130)), null);
    assert.equal(detectSaasFinancePaket([]), null);
  });
});
...
describe('Fortnox · rätt-storleks-rådgivning (verifierad prisskillnad)', () => {
  ...
  test('Mellan → Liten: 141 kr/mån × 12 = 1 692 kr/år', () => {
    const r = saasFinanceRightsizing(inv('Fortnox Paket Mellan', 490));
  ...
  test('Liten → Mini: 140 kr/mån × 12 = 1 680 kr/år', () => {
    const r = saasFinanceRightsizing(inv('Paket Liten', 349));
  ...
  test('Mini → null (redan billigaste paketet, inget att nedgradera)', () => {
    assert.equal(saasFinanceRightsizing(inv('Fortnox Mini', 209)), null);
  });
  test('inget igenkänt paket → null (ingen estimerad siffra når kund)', () => {
    assert.equal(saasFinanceRightsizing(inv('Visma eEkonomi Pro', 400)), null);
  });
