            ...
            {result.route === 'monitoring' ? (
              <>
                {result.recommendation?.netSaving > 0 && (
                  <SavingsBlock>
                    <span className="kicker">Potentiell nettobesparing när avtalet löper ut</span>
                    <span className="amount">+{formatKr(result.recommendation.netSaving)}</span>
                    <span className="unit">
                      Arvo agerar inför{' '}
                      {result.servicePeriodEnd
                        ? new Date(result.servicePeriodEnd).toLocaleDateString('sv-SE', { year: 'numeric', month: 'long' })
                        : 'avtalets slut'}{' '}
                      och sköter hela förhandlingen — utan kostnad tills ni sparar.
                    </span>
                  </SavingsBlock>
                )}
                <MonitoringBlock>
