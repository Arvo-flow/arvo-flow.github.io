...
export function byggAvstamningsrad(rad, kontext = {}) {
  ...
  //
  // Läsvägen är gemensam (lib/radobservation.js, 24 aug) men POLICYN är den här grindens egen:
  // `beloppOreAvlast` utesluter kronorfallbacken, för här är det avlästa talet hela beviset.
  // Balanskravet får använda konverteringen; en avstämning får inte. Skillnaden bor i valet av
  // fält, aldrig i en egen fältläsning — en egen läsning var precis det som gjorde balanskravet
  // blint i två dygn när aggregeringen döpte om fältet.
  const { beloppOreAvlast: ore, aprisOre } = radensOre(rad);
  ...
  if (Number.isInteger(aprisOre) && aprisOre > 0 && aprisOre * antal !== ore) {
