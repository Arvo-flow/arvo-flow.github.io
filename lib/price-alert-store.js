      ...
      db`
        ...
        WHERE category   = ${category}
          ...
          AND (user_email IS NULL OR NOT (
                  lower(trim(user_email)) = ANY(${TEST_EXAKTA})
                  OR (split_part(lower(trim(user_email)), '@', 2) = ${TEST_DOMAN}
                      AND split_part(split_part(lower(trim(user_email)), '@', 1), '+', 1) = ANY(${TEST_LOKALDELAR}))))
      `,
      db`
        SELECT COUNT(DISTINCT user_email) AS n
        FROM invoice_analyses   -- liggare: moat
