...
import { TEST_EXAKTA, TEST_DOMAN, TEST_LOKALDELAR } from './test-surface.js';
import { lasPremiumkrets, premiumFilter } from './premiumkrets.js';
...
//
// PREMIUMGRINDEN (grundarorder 2026-09-24): prislarm är ett proaktivt utskick och går BARA till
// premiumkretsen (lib/premiumkrets.js). Grinden sitter här, i den enda mottagarlistan, så att båda
// avsändarna (api/cron/run-price-alerts och scripts/notify-price-changes) omfattas av samma rad.
// En krets som inte kan läsas kastar genom catch nedan — aldrig «0 berörda».
export async function getAffectedCustomers({ supplierKeyword, category }, { db = getDb() } = {}) {
  if (!db) return [];

  try {
    // Steg 1: Hämta alla e-post ↔ rå-fingerprint-kopplingar — enbart premiumkretsens
    const allaRader = await db`
      SELECT email, fingerprint FROM gate_emails WHERE fingerprint IS NOT NULL
    `;
    const krets = await lasPremiumkrets(db);
    const { kvar: emailRows, utestangda } = premiumFilter(allaRader, krets);
    if (utestangda) console.log(`[premiumgrind] prislarm: ${utestangda} adresskoppling(ar) utanför premiumkretsen får inget larm`);
    ...
    // NOLL ÄR ETT PÅSTÅENDE, OKÄNT ÄR SANNINGEN (grundarfynd 2026-08-04).
    // Den här raden returnerade tidigare []. Under Neons kvotstopp 19–31 juli betydde det att
    // varje larmkörning skrev "👥 0 berörd(a) kund(er) · Klart: 0 skickade" och lyste grönt —
    // hade en verklig prishöjning detekterats hade INGEN kund fått veta det, och ingenting hade
    // sagt ifrån. Ett databasfel får aldrig förvandlas till ett faktapåstående om kundlistan.
    // Vi kastar i stället: anroparen måste hantera "jag vet inte", inte tro att svaret är noll.
    console.error('[price-alert-store] getAffectedCustomers error:', err.message);
    throw new Error(`Kunde inte läsa berörda kunder (${err.message}) — noll är inte ett svar här.`);
  }
}
...
// ── ETT MOAT-AGGREGAT FILTRERAR INTE ARKIVERADE RADER (rättat 2026-09-13) ────────────────────
// De här två satserna fick `arkiverad_at IS NULL` i 409b138, medan de tre andra moat-satserna
// inte fick det. Tre aggregat över samma liggare räknade alltså över OLIKA populationer, och två
// av dem sitter i SAMMA rum (api/invoice-history.mjs:432 och :609) — rumsredovisningens regel
// ordagrant: tal som står bredvid varandra ska gå att jämföra.
//
// Riktningen avgörs av grundarbeslutet 2026-09-11, som är uttryckligt: «analyser arkiveras, de
// raderas aldrig — PROVENIENSEN MÅSTE BESTÅ FÖR MOATENS SKULL». Arkivering betyder «borta ur
// KUNDENS EGEN VY», aldrig «har aldrig observerats». X/Y-talet är ett anonymt tvärkundsaggregat:
// det visar aldrig en enskild kunds rader tillbaka för hen, så filtret skyddade ingenting och
// underdrev vad vi faktiskt sett. Filtret borttaget; klassen `moat` kräver därför inget arkivfilter
// (LV-03 gäller `kundvy`, och det är två olika frågor).
