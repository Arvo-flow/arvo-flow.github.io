...
export function prissattningsdom(sammanfattning, facit) {
  // TRE TILLSTÅND, och de får aldrig se likadana ut (skärpt 2026-09-10, tredje granskningsvarvet).
  // Första versionen frågade `.some(...)`: EN nyckel räckte för att facit skulle räknas som giltigt,
  // och jämförelsen hoppade tyst över varje nyckel facit saknade. Bevisat av granskaren:
  //   prissattningsdom({fixturer:334, prissatt:0, tystad:0, offert:0, fel:0}, {fixturer:334})
  //     → blockerar:false, kod:'ok'   ← prissättningsgraden kollapsad till NOLL, godkänd
  // Det är felfamiljen spegelvänd: fixen dagen innan stängde hålet på MÄT-sidan och lämnade det
  // öppet på FACIT-sidan. Ett halvt facit är inte «inget facit» — det är ett TRASIGT facit, och de
  // två kräver motsatta åtgärder (frys en baslinje / laga filen).
  //   inget facit  → första körningen i en ny miljö. Blockerar inte.
  //   halvt facit  → baslinjen går inte att lita på. BLOCKERAR.
  //   helt facit   → jämför.
  // `Number.isFinite` i stället för `!== undefined` stänger samtidigt strängfallet: ett facit där
  // 334 blivit '334' gav texten «fixturer: 334 → 334» — ett larm ingen kan tolka.
  const narvarande = NYCKLAR.filter((k) => Number.isFinite(facit?.[k]));
  ...
  if (narvarande.length < NYCKLAR.length) {
    const saknade = NYCKLAR.filter((k) => !Number.isFinite(facit?.[k]));
    return {
      blockerar: true,
      kod: 'trasigt_facit',
      avvikelser: saknade.map((k) => `${k}: saknas i facit`),
      text: `FACIT ÄR OFULLSTÄNDIGT: ${saknade.join(', ')} saknas eller är inte tal. En baslinje med hål kan inte godkänna en mätning.`,
    };
  }
