...
//
// "Telia höjde priset — och X av Y bolag Arvo sett fakturor från för <kategori> ligger hos Telia."
// ⚠️ ORDET «FÖLJER» BYTT 2026-09-13. Talet kommer ur getSegmentStats, som (korrekt, enligt
// grundarbeslutet om proveniens) räknar ÄVEN arkiverade rader — medan mottagarlistan
// getAffectedCustomers är arkivfiltrerad. «Följer» lovade alltså en pågående bevakning för
// bolag vi slutat larma. «Sett fakturor från» är vad talet faktiskt mäter.
...
import { KOHORT_ENHET } from './kundmeningar.js';
...
export function marketMovementFinding(hike, segment, { supplier = null, categoryLabel = null, today = new Date(), recentMonths = 6, minPeers = 3 } = {}) {
  ...
  return {
    ...
    text: `${sup} höjde sitt publika pris med ${magnitude} den ${dateSv(when)} — verifierat. `
        + `${withSupplier} av ${total} ${KOHORT_ENHET} Arvo analyserat fakturor från för ${cat} ligger hos ${sup}; ni är en av dem. `
