...
//     motsatt besked (regel 5). ✅ SAMMANSLAGEN 2026-09-18, se lib/kategorinyckel.js.
 ...
 */
// ⚠️ SMALNAD I TVÅ STEG, BÅDA EFTER MÄTNING.
//
// STEG 1 (granskningens F3): texten lovade «vi bevakar avtalsslutet», «förbereder motbudet» och
// «säger till när något rör sig». Ingen av mekanikerna fanns för triagerade rader.
//
// STEG 2 (grundarens Q2, mätt 2026-09-18): ersättningen «Säg till när avtalet löper ut, så hör ni
// från oss 60 och 30 dagar innan» var BACKAD I DATALAGRET — save-contract skriver
// contract_end_date, send-reminders mejlar på 60/30 utan route-filter — men INTE I GRÄNSSNITTET.
// Bevakningskorten i rummet renderar `action` som ren text: noll input, noll knapp, noll länk,
// noll klickhanterare i hela Watched-blocket. En kund som läser «säg till» har ingenstans att
// säga det. En uppmaning utan väg är en återvändsgränd, och doktrinen skiljer inte på ett löfte
// utan mekanik och en uppmaning utan gränssnitt.
//
// Därför bär de tre bevakningsklasserna INGEN åtgärd alls. Kortet är ett konstaterande. Dagen
// gränssnittet finns läggs raden tillbaka — med mekaniken, inte före den.
// Karantänklassen behåller sin: «Ligger hos er försäkringsförmedlare» är ingen begäran till oss
// utan ett konstaterande om vem frågan tillhör, och kräver därför ingen knapp.
const BESKED = Object.freeze({
  [SKAL.VOLYMSTYRD]: {
    ...
    text: 'Kostnaden här styrs av volym och specifikation, inte av antalet anställda. Ett golv per '
      + 'anställd vore ett tal utan innebörd, så vi sätter inget.',
    atgard: null,
  },
  [SKAL.OFFERTPRISSATT]: {
    ...
    text: 'Priset i den här kategorin sätts i offert, inte på en publik prislista. Vi lovar därför '
      + 'aldrig ett byte här, och vi sätter ingen siffra vi inte kan belägga.',
    atgard: null,
  },
  // ⚖️ DEN ENDA KLASS UTAN ETT «VI»-LÖFTE. Varje annan text slutar med vad Arvo gör härnäst;
  // den här avgränsar i stället vårt mandat och lämnar frågan till någon annan. En förväntan som
  // inte nollas är en förväntan vi skapat, och för försäkring är det inte ett premiumfel utan
  // ett regulatoriskt.
  // ⚠️ FORMEN VAR FELET, INTE ORDEN (grundaren, fyra utkast 2026-09-17). Första versionen räknade
  // upp tre negationer — «vi prissätter inte, vi bevakar inte, vi hör inte av oss». Det är en
  // FRISKRIVNING, och en friskrivning låter inte exklusiv hur välformulerad den än är.  // pastaende-ok: omdöme om kopians form, inget anspråk om kodens beteende
  // Grundaren: «förminskar oss» · «för mesigt» · «vi låter inte professionella och exklusiva».
  //
  // Rättningen var strukturell: bort med uppräkningen, bort med motiveringen, bort med ursäkten.
  // «Lämnar varken bedömning, jämförelse eller bevakning» gör samma juridiska arbete i EN sats —
  // och är dessutom starkare legalt, eftersom den är kategorisk i stället för en lista som
  // inbjuder till «men vad gäller då X?». «Mandat» beskriver en avgränsning vi håller, inte ett
  // hål vi har.
  //
  // ⚠️ INGET ÅRTAL. Grundaren föreslog «vi räknar med tillstånd 2028». Det är regel 9 i sin
  // renaste form: en prognos om FI:s beslut, presenterad som ett löfte, på den enda yta där en
  // kund kan ta skada av att vänta. Ambitionen bokförs i `grund` (intern) och TS-14 förbjuder
  // varje årtal i kundtexten, så den inte kryper tillbaka nästa gång någon vill vara vänlig.
  [SKAL.TILLSTAND_KRAVS]: {
    rubrik: 'Utanför mandatet',
    rad: 'Försäkringar hanteras inte av Arvo',
    text: 'Försäkringsdistribution kräver tillstånd från Finansinspektionen. Arvo har inget i dag, '
      + 'och lämnar därför varken bedömning, jämförelse eller bevakning i den här kategorin.',
    atgard: 'Ligger hos er försäkringsförmedlare.',
  },
...
export const TYSTNADSSKAL = Object.freeze({
  ...
  'forsakring-foretag':  { skal: SKAL.TILLSTAND_KRAVS, grund: 'Tillståndspliktigt hos FI (försäkringsdistribution). Grundaren 2026-09-17: tillstånd väntas tidigast 2028 — INTERN ambition, får aldrig nå kundtexten (TS-14).' },
  'forsakring-ansvar':   { skal: SKAL.TILLSTAND_KRAVS, grund: 'Tillståndspliktigt hos FI (försäkringsdistribution). Samma interna tidslinje som företagsförsäkring; aldrig i kundtext.' },
  ...
  //
  //    ✅ `vaxel` STÅR INTE LÄNGRE HÄR (2026-09-18). Den var aldrig en egen kategori utan en
  //    avvecklad stavning av `molnvaxel`, som är real-public och TALAR. Nyckeln är sammanslagen
  //    (`lib/kategorinyckel.js`) och borta ur prisboken, alltså är den inte längre en TYST
  //    kategori som behöver ett skäl — den är ingen kategori alls. Att låta deklarationen stå
  //    kvar hade varit ett svar på en fråga som inte finns.
