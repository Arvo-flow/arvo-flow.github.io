...
const BESKED = Object.freeze({
  [SKAL.VOLYMSTYRD]: {
    ...
    text: 'Kostnaden här styrs av volym och specifikation, inte av hur många ni är — ett golv per '
      + 'anställd vore ett tal utan innebörd. Vi bevakar avtalet mot förfallodatum och går igenom '
      + 'underlaget manuellt inför omförhandling.',
  },
  [SKAL.OFFERTPRISSATT]: {
    rubrik: 'Offertprissatt',
    text: 'Det finns inget publikt listpris att mäta er mot i den här kategorin — priset sätts i '
      + 'offert. Vi lovar därför aldrig ett byte här. Vi bevakar avtalsslutet och förbereder '
      + 'fyndet, tajmingen och det exakta motbudet inför omförhandlingen.',
  },
  [SKAL.UTLANDSK_VALUTA]: {
    rubrik: 'Publikt pris — i utländsk valuta',
    text: 'Leverantörerna publicerar sina priser, men inte i kronor. Ett SEK-belopp räknat ur en '
      + 'utländsk prislista via dagskurs är ingen verifierad siffra, och vi visar den inte. Vi '
      + 'bevakar prisändringarna i källvalutan och säger till när något rör sig.',
  },
});
...
export function tystnadsbesked(kategori) {
  ...
  return { skal: post.skal, rubrik: besked.rubrik, text: besked.text };
}
