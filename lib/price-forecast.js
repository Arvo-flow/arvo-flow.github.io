...
export function priceHikeForecast(rows, { supplier = null, today = new Date() } = {}) {
  // Behåll bara verkliga, REDAN INTRÄFFADE höjningar med giltigt datum och positiv magnitud.
  // En framtidsdaterad rad (en ANNONSERAD men ej ikraftträdd ändring) är inte mönsterbevis —
  // det är ett känt faktum om morgondagen, inte ett prejudikat om gårdagen. Att låta den räknas
  // som "historik" blandar ihop precisionsfacket (vi VET) med bedömningsfacket (vi GISSAR) —
  // exakt den kategori-förväxling som gjorde Microsoft-kortet missvisande (grundarlärdom 2026-06-28).
  const hikes = [];
  for (const r of rows ?? []) {
    const when = r?.changed_at ? new Date(r.changed_at) : null;
    if (!when || Number.isNaN(when.getTime()) || when > today) continue;
  ...
  // Zero Trust-grind: samma golv som kohorten (lib/price-alert.js) och Marknadsrörelsen
  // (lib/market-movement.js) — under TRE höjningar är två punkter en linje, inte ett mönster,
  // och "trend"-språk skulle överdriva vad vi faktiskt vet (grundarlärdom 2026-06-28: Microsofts
  // två höjningar, 11 år isär följt av 4 år, är motsatsen till en regelbunden kadens).
  if (hikes.length < 3) return null;
  ...
  const asymmetri = `Vi bevakar fönstret och säger till i tid. Kommer höjningen inte — desto bättre; ni betalade inget för beredskapen och bevakningen flyttar bara fram.`;
  ...
  return {
    ...
    // Strukturerade delar — bedömningskravet (scripts/bedomningskrav.mjs) verifierar att alla tre
    // finns OCH når kunden i texten. En bedömning utan någon av dem får aldrig produceras.
    basis: grund,
    asymmetry: asymmetri,
    confidence,
