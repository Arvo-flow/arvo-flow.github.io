...
// ── WORKERN MÅSTE OCKSÅ IN I BUNDLEN (2026-09-10) ────────────────────────────────────────────
// MÄTT I PRODUKTION efter att DOMMatrix-fixen deployats. Polyfillen fungerade — canvas-felet var
// borta — men textlagret var fortfarande dött, nu med ett HELT ANNAT fel:
//
//     [textlager] kunde inte läsas — ingen kolumnkorrigering: Setting up fake worker failed:
...
//
// Samma familj som canvas, ett steg in: pdfjs kör utan riktig worker i Node och laddar därför sin
// worker-modul DYNAMISKT («fake worker»). Vercels filspårare följer statiska importer och såg
// aldrig den, så filen kopierades aldrig in i funktionens `/var/task/node_modules`.
//
// ⚠️ ATT LÄGGA `pdfjs-dist/**` I `includeFiles` GÖRS OCKSÅ (vercel.json), men det ensamt är inte
// det som bevisats fungera — canvas-fallet visade att spåraren är den som avgör vad som faktiskt
// packas. Därför importeras workern HÄR, explicit, precis som canvas. Modulen är den pdfjs själv
// hade importerat; ES-modulcachen gör att pdfjs får samma instans utan att ladda den två gånger.
//
// Misslyckas importen loggas SKÄLET och vi fortsätter: pdfjs får då försöka sin egen väg, och ett
// tyst fall-through var precis det som gjorde att orsaken var okänd i fyra dygn.
let _workerLaddad = null;
async function sakerstallWorker() {
  if (_workerLaddad !== null) return _workerLaddad;
  try {
    await import('pdfjs-dist/legacy/build/pdf.worker.mjs');
    _workerLaddad = true;
  } catch (err) {
    console.error('[pdf-textlager] pdf.worker.mjs kunde inte förladdas:', err.message);
    _workerLaddad = false;
  }
  return _workerLaddad;
}
