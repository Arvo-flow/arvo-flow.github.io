...
function toKrPerSeatMonth(numeric, currency, unit, seats, fxRates) {
  ...
  switch (unit) {
    ...
    // ── EN OKÄND ENHET ÄR ODÖMBAR (2026-08-24, H2 fall C/D/G) ────────────────────────────────
    // Här stod `default: return kr` — en okänd eller icke-hanterad enhet antogs vara
    // kr/säte/månad. `percentage` står i Haikus EGEN enum och saknar case: 1,95 % blev
    // 1,95 kr/säte/mån. Och `per_month` (utanför enumet) gjorde 3 588 kr/ÅR till 3 588 kr/MÅN
    // → «+1 578 720 kr/år» i ett kundmail. computeImpactKr returnerar redan null när någon
    // sida är odömbar, så tystnaden propagerar korrekt.
    default:                  return null;
  }
}
