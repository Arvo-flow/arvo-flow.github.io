...
export async function pollExtractBatch(batchId) {
  ...
  for await (const item of await client.beta.messages.batches.results(batchId)) {
    ...
    try {
      ...
      // `null` = inget dokument här. Batch-API:t ger tillbaka modellens svar, aldrig PDF:en, så
      // kolumnläsaren har inget att läsa ur — och det ska SÄGAS, inte utelämnas (fynd 4).
      // Konsekvens, uttalad: fakturor som går den här vägen får `seatCount` ur modellens tal
      // precis som före 8 sep. Vill vi ha pappret även här måste bytes bäras hit.
      const aggregated = aggregateLineItems(block.input, null);
