...
export function molnvaxelRecommendation(input) {
  ...
  const bundled = normalized.bundled === true;             // växelpris buntar mobilsurf → ej jämförbart
  // Bundlat pris jämförs ALDRIG mot ett rent växelgolv (äpplen-mot-päron, pilotdata-läxan).
  const floor = bundled ? null : (tv.tiers[tier]?.fromMonthly ?? null);  // T1=89, T2=118, T3/bundlat=null
  ...
  const perBolagMonthly = normalized.perBolagMonthly ?? 0;
  ...
  if (bundled) {
    reasoning =
      `Ni betalar ${fmt2(perUser)} kr/användare och månad (exkl moms) för ett paket som buntar växel OCH ` +
      `mobilabonnemang. Det priset inkluderar mobilsurf och är därför inte direkt jämförbart med en ren ` +
      `växellicens — vi jämför mot ert faktiska pris i en genomgång istället för att visa en missvisande siffra. `;
  } else {
    reasoning = `Ni betalar ${fmt2(perUser)} kr/användare och månad (exkl moms) för ${seats} växellicenser på ${CANONICAL_TIERS[tier].label}-nivån. `;
    // ⚠️ BOLAGSAVGIFTEN FÅR INTE FÖRSVINNA. Reception/IVR/svarsgrupp ligger utanför divisionen —
    // de prissätts inte per användare — men de är en kostnad kunden betalar. Att bara lyfta ut
    // dem ur täljaren och tiga hade underskattat vad växeln kostar. Talet kommer ur fakturans
    // egna rader (lib/vaxelrad.js), aldrig ur en uppskattning.
    if (perBolagMonthly > 0) {
      reasoning += `Utöver licenserna betalar ni ${fmt0(perBolagMonthly)} kr/mån för `
        + `bolagsgemensamma växeltjänster — de ingår inte i priset per användare. `;
    }
    if (floor != null) {
  ...
  return {
    ...
    molnvaxel: {
      ...
      overFloorPct, addons, bundled, perBolagMonthly,
      // ⚠️ DET NORMALISERADE UNDERLAGET BÄRS VIDARE, så att moaten kan lagra EXAKT det tal kunden
      // såg. Alternativet — att api-lagret anropar normalizeTelekomInvoice en andra gång — hade
      // gett två beräkningar av samma sak, och två beräkningar kan glida isär (regel 1). Fältet
      // är internt: det når prisboken via buildTelekomDatapoint, aldrig en kundyta.
      normaliserad: normalized,
      excludedMobilMonthly: normalized.excludedMobilMonthly ?? 0,
      excludedOvrigMonthly: normalized.excludedOvrigMonthly ?? 0,
    },
