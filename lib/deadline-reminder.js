...
export function deadlineReminderDecision({ terms, marker = null, today = new Date() }) {
  ...
  // Kvitteringen tystar mejlen (grundardesign 2026-07-09): 'uppsagd' → aldrig mer larm
  // (nedräkningen äger nu); 'stannar' → tyst för EXAKT den kvitterade deadlinen —
  // nästa period matchar inte nyckeln och varnas som vanligt.
  if (view.kundStatus?.typ === 'uppsagd') return { send30: false, send7: false, marker, view };
  if (view.stannarAktiv) return { send30: false, send7: false, marker, view };

  return { ...varselFranDeadline({ deadline: view.clock.deadline, daysToDeadline: view.clock.daysToDeadline, marker, today }), view };
}

/** Dagarna före sista uppsägningsdag då ett varsel går. EN definition för avtalsvyn OCH fakturaklockan. */
export const VARSEL_DAGAR = [30, 7];

/**
 * Varselbeslutet mot en KÄND sista uppsägningsdag — delat av avtalsvyn (ovan) och fakturaklockan
 * (lib/paminnelse.js), så att de två vägarna aldrig kan lova olika datum.
 */
export function varselFranDeadline({ deadline, daysToDeadline, marker = null, today = new Date() }) {
  const d = daysToDeadline;
  // Ny period? Deadline-datumet har rullat → färsk markör (rullande avtal påminns per period).
  const m = (marker && marker.deadline === deadline)
    ? { ...marker }
    : { deadline };

  const [forsta, sista] = VARSEL_DAGAR;
  const send7  = d <= sista && d >= 0 && !m.sent7;
  const send30 = d <= forsta && d > sista && !m.sent30;

  if (send30) m.sent30 = today.toISOString();
  if (send7) { m.sent7 = today.toISOString(); m.sent30 = m.sent30 ?? today.toISOString(); }

  return { send30, send7, marker: m };
}
