 ...
 */
/**
 * En prisändring som en LÄSBAR RAD — med hela identiteten kvar.
 *
 * ⚠️ VARFÖR DEN HÄR FUNKTIONEN FINNS. Arkeologin mot Loopia gav raden
 *
 *     ↓ Webbhotell Privat: 295 → 0 kr (-100 %) · 2020-10-21 → 2021-01-26
 *
 * som läses «produkten blev gratis». Datan var HELT KORREKT — paret var
 * `Webbhotell Privat · Startavgift → 295 kr`, och Loopia tog bort sin startavgift. Det var
 * UTSKRIFTEN som tappade `rad` och därmed gjorde ett sant tal till ett falskt påstående.
 *
 * Samma felfamilj som MK-08 («Billigaste publicerade pris 1 606 kr» på en Google-rad), fast i
 * presentationslagret: ett tal utan sin kontext blir en annan utsaga än den mätningen bär.
 * Jag var nära att rapportera en parsningsbugg som inte fanns. (PP-13, 2026-09-05.)
 */
export function beskrivAndring(a) {
  const tecken = a.tillPris > a.franPris ? '↑' : '↓';
  const namn = a.rad ? `${a.paket} · ${a.rad}` : a.paket;
  const pct = a.procent == null ? '—' : `${a.procent > 0 ? '+' : ''}${a.procent} %`;
  return `${tecken} ${namn}: ${a.franPris} → ${a.tillPris} kr (${pct}) · ${a.fran} → ${a.till}`;
}
