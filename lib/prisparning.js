...
export function prisandringar(avlasningar = []) {
  const serie = new Map();                      // paket → [{datum, pris}]
  for (const { datum, par } of avlasningar) {
    for (const p of par ?? []) {
      if (!Number.isFinite(p?.listpris)) continue;
      if (!serie.has(p.paket)) serie.set(p.paket, []);
      serie.get(p.paket).push({ datum, pris: p.listpris });
    }
  }
  const ut = [];
  for (const [paket, punkter] of serie) {
    for (let i = 1; i < punkter.length; i += 1) {
      ...
      ut.push({
        ...
        franPris: a.pris,
        tillPris: b.pris,
        procent: Math.round(((b.pris - a.pris) / a.pris) * 1000) / 10,
      });
    }
  }
  return ut;
