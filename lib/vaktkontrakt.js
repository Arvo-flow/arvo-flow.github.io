...
export function bedomVaktkontrakt(v) {
  ...
  // ── KONTRAKTETS FJÄRDE FRÅGA (2026-08-22): VILKEN KATEGORIPOST DATERAR VAKTEN? ─────────────
  // `bevakadeTiers` beskriver vilka LICENSNIVÅER vakten läser. Men prisboken har också ett datum
  // per KATEGORI (top-level lastVerified), och det ruttnade i tysthet: molnvaxel låg 66 dagar
  // gammal medan telia-vaxel-vakten körde veckovis och bekräftade priset varje gång. Ingen
  // frågade vem som daterade kategoriposten, så ingen gjorde det.
  //
  // `null` är ett giltigt SVAR — men svaret måste finnas. Skillnaden mellan «vaktar ingen
  // kategoripost» och «ingen frågade» ska stå i koden, precis som för bevakadeTiers.
  // EN kontroll, inte två. Första versionen hade en `in`-gren för «fältet saknas» och en
  // `else if` för «fältet är tomt» — men `undefined` fångas av den andra ändå, så den första var
  // redundant. Sabotaget avslöjade det: att ta bort `in`-grenen ändrade ingenting, och mitt test
  // var grönt på fel grund. Två grenar som ser ut som två lager men är ett räknar ett skydd vi
  // inte har (samma läxa som drift-backstopen i verifieringsstampel.js).
  if (v?.bevakadKategori !== null && (typeof v?.bevakadKategori !== 'string' || !v.bevakadKategori.trim())) {
    brister.push(`${id}: 'bevakadKategori' saknas eller är tom — vilken kategoripost daterar vakten? (null om ingen)`);
  }
