...
/**
 * VAR STÅR RUMSADRESSEN? (2026-09-25, andra blickens fynd 5). Efter en vidarebefordringsregel är To-rubriken
 * kundens egen adress; rumsadressen står bara i kuvertet eller i en leveransrubrik. Vilket fält Resend fyller
 * är OBEKRÄFTAT (probe-rumsadress, fallet B) — därför läses varje kandidat i en fast ordning, och fältet som
 * träffade rapporteras så att sonden kan läsa det. Returnerar { nyckel, falt } eller null.
 */
const RUBRIKER = ['delivered-to', 'x-original-to', 'x-forwarded-to', 'envelope-to', 'to', 'cc'];
export function mottagarnyckel(mejl) {
  if (!mejl || typeof mejl !== 'object') return null;
  const rubrik = (namn) => {
    const h = mejl.headers;
    if (!h) return null;
    if (Array.isArray(h)) return h.filter((x) => String(x?.name ?? '').toLowerCase() === namn).map((x) => x.value);
    const k = Object.keys(h).find((x) => x.toLowerCase() === namn);
    return k ? h[k] : null;
  };
  const kandidater = [
    ['to', mejl.to], ['cc', mejl.cc], ['bcc', mejl.bcc], ['recipients', mejl.recipients],
    ['envelope.to', mejl.envelope?.to], ['envelope.recipients', mejl.envelope?.recipients],
    ...RUBRIKER.map((r) => [`rubrik:${r}`, rubrik(r)]),
  ];
  for (const [falt, varde] of kandidater) {
    const nyckel = nyckelUrMottagare(varde);
    if (nyckel) return { nyckel, falt };
  }
  return null;
}
...
export async function adressForRum(db, { rumsnyckel = null, agareEpost = null, plattform = null } = {}) {
  ...
  if (epost) {
    ...
    // En ENHETS adress knyts aldrig till en e-post, inte ens när den saknar ägare: på en delad dator hade
    // den som först loggar in tagit över en annan persons vidarebefordran — fakturor, svar och rum (IA-08).
    // Den bevisade e-posten får en egen adress; enhetens adress står kvar hos enheten.
  } else {
  ...
  // En ÄGD adress får aldrig enhetens hash: annars hittar nästa utloggade person på samma dator ägarens
  // adress via rumsnyckeln — fakturor, Gmail-kod och «Försök igen» (andra blicken 2, IA-08).
  // Två samtidiga förfrågningar för samma ägare eller enhet: den unika indexet avgör, förloraren läser
  // vinnarens rad i stället för att skapa en andra adress som rummet aldrig läser (IA-16).
  const [ny] = await db`
    INSERT INTO inkorgsadresser (nyckel, rum_hash, agare_epost, plattform)
    VALUES (${nyckel}, ${epost ? null : hash}, ${epost}, ${plattform})
    ON CONFLICT DO NOTHING
    RETURNING *
  `;
  if (ny) return ny;
  const [vinnare] = epost
    ? await db`SELECT * FROM inkorgsadresser WHERE agare_epost = ${epost} ORDER BY skapad_at ASC LIMIT 1`
    : await db`SELECT * FROM inkorgsadresser WHERE rum_hash = ${hash} LIMIT 1`;
  if (!vinnare) throw new Error('inkorgsadress: adressen kunde varken skapas eller läsas');
  return vinnare;
}
...
/**
 * Rummets läsväg (api/invoice-history). Bara en SAKNAD TABELL — glappet mellan deploy och migrering — läses
 * som «ingen adress», för då kan ingen adress finnas. Allt annat kastas vidare till rummets 503: ett läsfel
 * får aldrig se ut som ett rum utan adressens fakturor (IA-12).
 */
export async function rumsadressForLasning(db, id) {
  try {
    return await hittaRumsadress(db, id);
  } catch (err) {
    if (!/relation "inkorgsadresser" does not exist/.test(String(err?.message))) throw err;
    console.warn('[inkorgsadress] tabellen saknas (migrering ej körd) — rummet visas utan egen adress');
    return null;
  }
}
