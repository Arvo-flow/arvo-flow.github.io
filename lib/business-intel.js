...
// Fynden — REN funktion (testbar). listPrice = det VERIFIERADE publika listpriset
// (BRANCHINDEX licenseTierBenchmarks: kr/anv/MÅN på årsavtal, källa microsoft.com).
// GRUNDARLÄRDOM 2026-07-01: förra versionen använde p25 (1 704 kr/anv/år — ett benchmark-
// ESTIMAT) men etiketterade det "verifierat listpris" — regel 3-brott (fel proveniens på ett tal).
// Nu bär raden det äkta listpriset, exakt som publicerat. Deterministisk aritmetik i kod (regel 2).
export function buildBusinessFindings(facts, { listPrice } = {}) {
  if (!facts) return [];
  const f = [];
  ...
  const perMonth = Number(listPrice?.monthlyPerUserAnnualCommit);
  if (Number.isFinite(perMonth) && perMonth > 0) {
    const floorAnnual = facts.employees * perMonth * 12;
    f.push({
      kind: 'costline', confidence: 'high',
      title: `Era kontorslicenser: från ~${fmtNum(floorAnnual)} kr/år vid verifierat listpris`,
      detail: `${fmtKr2(perMonth)} kr/användare/mån (${listPrice.product ?? 'Microsoft 365 Business Standard'}, årsavtal — verifierat publikt listpris) × era ${facts.employees} anställda × 12 mån. De flesta betalar mer — dela en faktura så ser vi exakt var ni ligger.`,
      source: `Verifierat publikt listpris (microsoft.com${listPrice.lastVerified ? `, ${listPrice.lastVerified}` : ''}) × antal anställda ur årsredovisningen`,
    });
  }
  return f;
}
