...
// Fynden — REN funktion (testbar). p25PerUser = verifierat golvpris kr/anv/år (BRANCHINDEX,
// enhetligt över segment — testlåst). Deterministisk aritmetik i kod (regel 2), källa på varje rad (regel 3).
export function buildBusinessFindings(facts, { p25PerUser } = {}) {
  if (!facts) return [];
  const f = [];
  f.push({
    ...
    detail: 'Vi läser era förutsättningar innan vi läser era fakturor — offentliga uppgifter, inget ni behövt dela.',
    source: `Offentliga årsredovisningsuppgifter (Bolagsverket) · bokslutsår ${facts.year}`,
  });
  if (Number.isFinite(p25PerUser) && p25PerUser > 0) {
    const floorAnnual = facts.employees * p25PerUser;
    f.push({
      kind: 'costline', confidence: 'high',
      title: `Era kontorslicenser: från ~${fmtNum(floorAnnual)} kr/år vid verifierat golvpris`,
      detail: `${fmtNum(p25PerUser)} kr/användare/år (verifierat golvpris) × era ${facts.employees} anställda. De flesta betalar mer — dela en faktura så ser vi exakt var ni ligger.`,
      source: 'Verifierat publikt listpris (Microsoft 365) × antal anställda ur årsredovisningen',
    });
  }
  return f;
}
