...
// TVÅ vägar i ordning (Kristianstad-läxan): (1) orgnr publicerat på bolagets egen sajt → exakt
// uppslag utan namnmatchning (klarar å/ä/ö-namn och juridiska namn som inte liknar domänen);
// (2) allabolag-sök + konservativ namngrind (den ursprungliga vägen, kvar som fallback).
export async function fetchBusinessFacts(domain, { fetchImpl = fetch, timeoutMs = 8000 } = {}) {
  const sld = sldFromDomain(domain);
  if (!sld) return null;

  try {
    const siteOrgnr = await fetchOrgnrFromWebsite(domain, { fetchImpl });
    if (siteOrgnr) {
      const facts = await fetchBusinessFactsByOrgnr(siteOrgnr, { fetchImpl, timeoutMs });
      if (facts) return facts;
    }
  } catch { /* vidare till sökvägen */ }
