...
export function extractCompanyFacts(nd) {
  ...
  if (!c) return null;
  // ── ÅRET KOMMER UR RÄKENSKAPSÅRETS EGET FÄLT (rättat 2026-08-12) ────────────────────────────
  // Raden hämtade året ur companyAccountsLastUpdatedDate — när POSTEN senast uppdaterades — och
  // skrev ut det som "Ert bokslut 2024". Det är en annan storhet än räkenskapsåret; att de ofta
  // sammanfaller gör inte den ena till den andra. Grundaren fångade det på ett kort där Avida sa
  // 2024 och Skanska 2025, och samma kort bar redan det äkta årtalet en rad längre ned
  // (trendraden läser companyAccounts[].year).
  //
  // Men det räckte inte att byta fält: etiketten och TALEN måste komma ur SAMMA post, annars
  // byter man bara ut ett felmärkt tal mot ett annat. Rubrikens c.revenue kan avse ett annat år
  // än det vi skriver ut. Därför byggs bokslutsraden nu av ETT bokslut — år, omsättning och
  // anställda ur samma rad i historiken. Saknas en del utelämnas den; den lånas aldrig från en
  // annan post för att raden ska se komplett ut.
  //
  // Utan historik finns inget källbelagt år. Då står rubrikens tal kvar men året utelämnas —
  // hellre "senaste bokslut" utan årtal än ett årtal som betyder något annat (regel 3/4).
  const history = extractAccountHistory(c.companyAccounts);
  const senaste = history[0] ?? null;
  const revenueTkr = Number(senaste ? senaste.revenueTkr : c.revenue);
  // Number(null) är 0, och 0 föll rakt i grinden för "orimligt antal" nedan — ett SAKNAT värde
  // hade alltså tystat hela raden. Saknat ska förbli saknat (NaN), aldrig bli en nolla som råkar
  // vara ett tal. Samma fälla som momsbasen: ett default som ser ut som en avläsning.
  const employees  = senaste
    ? (Number.isFinite(senaste.employees) ? Number(senaste.employees) : NaN)
    : Number(c.employees);
  const year       = senaste ? senaste.year : null;
  if (!Number.isFinite(revenueTkr) || revenueTkr <= 0) return null;
  // Anställdatalet får saknas i ett bokslut (ANT-koden är inte alltid ifylld). Då tystnar just
  // den delen av meningen — men bara när talet kommer ur historiken. Rubrikvägen behåller sin
  // gamla, strängare grind, eftersom där finns inget år som binder talet till en period.
  if (senaste) {
    if (Number.isFinite(employees) && (employees <= 0 || employees > 100000)) return null;
  } else if (!Number.isFinite(employees) || employees <= 0 || employees > 100000) return null;
  ...
  return {
    ...
    legalName: c.legalName ?? null, orgnr: c.orgnr ?? null, revenueTkr,
    employees: Number.isFinite(employees) ? employees : null, year,
...
// Identitetskortet blev en dossier — bokslut, tillväxt, koncern, ålder — och bar noll kronor.
// Fem rader om VILKA de är, ingen om vad något KOSTAR. Vi bevisade att vi kan läsa ett register,
// aldrig att vi vet något om deras pengar. Moaten syntes inte med ett tecken.
//
// Den här raden är den enda pengarad som går att säga innan en faktura delats, utan att gissa:
// den multiplicerar TVÅ verifierade fakta — bolagets egna anställdatal ur bokslutet och Microsofts
// publika listpriser, vaktade veckovis — och redovisar AVSTÅNDET mellan billigaste och dyraste
// licensnivå. Ingenting sägs om vad kunden betalar; spannet är ett faktum om prislistan, inte ett
// påstående om fakturan. Det är skillnaden mellan "ni överbetalar X" (förbjudet utan underlag)
// och "X hänger på ett val ni gjort utan att se prislappen" (sant, kontrollerbart, och stort).
//
// Regel 3: varje tal går att räkna hem med miniräknare ur källan. Regel 4: ingen påstådd nivå.
export function buildLicensspannFinding(facts, tierBenchmarks) {
  const anstallda = facts?.employees;
  if (facts?.provenance !== 'bolagsverket') return null;      // samma invariant som bolagsraderna
  if (!Number.isFinite(anstallda) || anstallda < 5) return null;   // under fem anställda är spannet inte dramat
  const nivaer = Object.values(tierBenchmarks ?? {})
    .filter((t) => t && t.currency === 'SEK' && Number.isFinite(t.msrpAnnual) && t.msrpAnnual > 0);
  if (nivaer.length < 2) return null;                          // utan spann finns ingen rad

  const priser = nivaer.map((t) => t.msrpAnnual).sort((a, b) => a - b);
  const lagst = priser[0], hogst = priser.at(-1);
  const perAnstalldAr = Math.round((hogst - lagst) * 12);
  const totalt = perAnstalldAr * anstallda;
  const datum = nivaer.map((t) => t.lastVerified).filter(Boolean).sort().at(-1) ?? null;

  return {
    kind: 'licensspann', confidence: 'high',
    title: `${fmtKr(totalt)} kr/år skiljer billigaste från dyraste licensnivå — för era ${anstallda} anställda`,
    detail: `Microsofts publika listpriser spänner ${fmtKrDec(lagst)}–${fmtKrDec(hogst)} kr per användare och månad. `
      + `Vilken nivå ni ligger på står bara på fakturan — men avståndet är ${fmtKr(perAnstalldAr)} kr per anställd och år. `
      // SISTA MENINGEN, RÄTTAD INNAN DEN NÅDDE NÅGON: första versionen skrev "det valet gjordes en
      // gång av någon som inte såg den här summan". Det är ett påstående om kundens interna historia
      // som vi omöjligt kan belägga — och som en CFO som valde medvetet kan falsifiera på en sekund.
      // Samma gräns som SPF-raden: säg bara det som är ofalsifierbart sant. Spannet är ett faktum
      // om prislistan; vad de valt och varför vet vi inte, och vi låtsas inte att vi gör det.
      + `Vi påstår inget om er nivå — vi visar vad den är värd.`,
    source: `Microsofts publika listpriser${datum ? `, verifierade ${datum}` : ''} × ert anställdatal ur bokslutet`,
  };
}

const fmtKr = (n) => Math.round(n).toLocaleString('sv-SE');
const fmtKrDec = (n) => n.toFixed(2).replace('.', ',');
...
export function buildBusinessFindings(facts, { now = new Date() } = {}) {
  ...
  f.push({
    ...
    // ── RADEN TÅL ETT SAKNAT ÅR OCH ETT SAKNAT ANSTÄLLDATAL (2026-08-12) ─────────────────────
    // Året kommer nu ur räkenskapsårets eget fält. Finns ingen bokslutshistorik finns inget
    // källbelagt år — då skriver vi "Ert senaste bokslut" i stället för att sätta dit en
    // uppdateringsstämpel som ser ut som ett årtal. Och saknas ANT-koden i bokslutet utelämnas
    // anställdatalet; det lånas aldrig från en annan post för att meningen ska bli hel.
    title: `Ert ${facts.year ? `bokslut ${facts.year}` : 'senaste bokslut'}: ${fmtMkr(facts.revenueTkr)} mkr i omsättning`
      + (Number.isFinite(facts.employees) ? `, ${facts.employees} ${facts.employees === 1 ? 'anställd' : 'anställda'}` : ''),
    ...
    source: facts.year ? `Bolagsverket · bokslutsår ${facts.year}` : 'Bolagsverket · senast inlämnade bokslut',
  });
  ...
  if (h.length >= 2 && h[0].revenueTkr > 0 && h[1].revenueTkr > 0) {
    ...
    if (Math.abs(pct) >= 5) {
      ...
      f.push({
        ...
        detail: rising
          ? `${spann} Om avtalen har vuxit i samma takt syns först i fakturorna — dela dem, så läser vi dem exakt.`
  ...
  if (fy && age >= 20) {
    f.push({
      ...
      detail: `Ett bolag med ${age} år bakom sig kan ha avtal som aldrig omprövats. Om era har det ser vi först i fakturorna.`,
  ...
  // kundmening-ok: koncernens bolag ur Bolagsverket — en registeruppgift, inte en kohort (PROVENIENS_OCH_ENHET)
  const koncernBolag = (n) => `${n} bolag`;
  const STORKONCERN_GRANS = 25;
  if (k && k.companies >= STORKONCERN_GRANS) {
    f.push({
      kind: 'koncern', confidence: 'high',
      title: k.parentName ? 'Ni ägs av en större koncern' : `Er koncern rymmer ${koncernBolag(k.companies)}`,
      // "NÄRMASTE ÄGARE", inte "moderbolag" (2026-08-12, efter sondmätning på Skanska Sverige AB).
      // Registret uppger Skanska Kraft AB — ett mellanled mellan driftbolaget och den noterade
      // toppen Skanska AB. Raden var SANN men ordet "moderbolag" fick den att läsa som en miss för
      // den enda läsare som vet bättre: bolaget självt. "Närmaste ägare" är exakt vad fältet
      // betyder, och förvandlar ett "va?" till ett "hur visste de det?" — mellanledet känner nästan
      // ingen utanför koncernen till.
      detail: `${k.parentName ? `Närmaste ägare i strukturen: ${k.parentName} — ` : ''}${koncernBolag(k.companies)} i ägarstrukturen. Ert bokslut och era avtal läser vi för er juridiska enhet — aldrig för portföljen.`,
      source: `Bolagsverket · ${koncernBolag(k.companies)} i strukturen`,
    });
  } else if (k && k.companies >= 2) {
    f.push({
      kind: 'koncern', confidence: 'high',
      title: k.parentName
        ? `Ni ingår i en koncern om ${koncernBolag(k.companies)}`
        : `Er koncern rymmer ${koncernBolag(k.companies)}`,
      detail: `${k.parentName ? `Moderbolag: ${k.parentName}. ` : k.subsidiaries ? `${k.subsidiaries} dotterbolag. ` : ''}Vi läser avtalen för varje juridisk enhet för sig.`,
      source: `Bolagsverket · ${koncernBolag(k.companies)} i strukturen`,
    });
  }
  return f;
...
export function mergeRevealFindings(businessFindings, dnsFindings, marketAnchor = null) {
  ...
  // Pengaraden rankas direkt efter identiteten och FÖRE plattform/koncern/ålder: ett kort som
  // öppnar med vilka de är och sedan säger vad det kostar läser som ett underlag; samma rader i
  // omvänd ordning läser som bakgrundsfakta med en prislapp påklistrad sist.
  const RANK = { business: 0, trend: 1, licensspann: 1.5, cross: 2, platform: 3, suppliers: 4,
    spoofing: 5, koncern: 6, heritage: 7, onboarding: 8, domain: 9, cert: 10, dmarc: 11,
...
async function searchAndMatch(query, sld, fetchImpl, timeoutMs, { allowPrefix = true, samla = null } = {}) {
  ...
  if (samla) {
    // ── NAMNET SÄGER VAD TALET ÄR (2026-08-12) ────────────────────────────────────────────────
    // Hette 'lasta' och nådde kundytan som "25 LÄSTA" — ett kvitto på vårt arbete. Mätningen
    // (ops/probe-soktraffar.txt) visade att söksidan har ett TAK på 25: skanska, avida och
    // westander gav alla exakt 25, trivector 17, ett nonsensord 0. Det finns inte 25 bolag som
    // heter Westander; sidan fyller sin sida med luddiga träffar.
    // Talet är alltså en sidstorlek, aldrig ett mått på hur mycket vi läst. Det heter nu det det
    // är, så att ingen framtida yta kan råka lova något med det.
    samla.sidtraffar = Math.max(samla.sidtraffar ?? 0, bolag.length);
