...
export function identityCandidates(sld, companies) {
  ...
  return companies
    ...
    .map((c) => ({
      ...
      // "Närmast er domän" är ett FAKTUM om stavning, inte ett påstående om ägarskap.
      // Därför får det märkas — men aldrig väljas åt kunden.
      closest: foldToDomainAlphabet(normalizeCompanyName(c.legalName ?? c.name)) === target,
    }));
}
