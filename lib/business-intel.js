...
export function buildBusinessFindings(facts) {
  ...
  if (h.length >= 2 && h[0].revenueTkr > 0 && h[1].revenueTkr > 0) {
    ...
    if (Math.abs(pct) >= 5) {
      const spann = `Från ${fmtMkr(h[1].revenueTkr)} till ${fmtMkr(h[0].revenueTkr)} mkr (bokslutsåren ${h[1].year} → ${h[0].year}) — offentliga bokslutssiffror.`;
      f.push({
        ...
        source: `Offentliga årsredovisningsuppgifter (Bolagsverket) · bokslutsåren ${h[1].year} och ${h[0].year}`,
      });
    }
  }
  return f;
