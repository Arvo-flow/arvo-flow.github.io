...
async function searchAndMatch(query, sld, fetchImpl, timeoutMs, { allowPrefix = true } = {}) {
  const sRes = await fetchImpl(`https://www.allabolag.se/what/${encodeURIComponent(query)}`,
    { headers: HEADERS, redirect: 'follow', signal: AbortSignal.timeout(timeoutMs) });
  if (!sRes.ok) return null;
  const match = matchCompany(sld, extractSearchCompanies(extractNextData(await sRes.text())), { allowPrefix });
...
export async function fetchBusinessFacts(domain, { fetchImpl = fetch, timeoutMs = 8000 } = {}) {
  ...
  for (const [q, allowPrefix] of [[sld, true], ...widened.map((q) => [q, false])]) {
    try {
      const facts = await searchAndMatch(q, sld, fetchImpl, timeoutMs, { allowPrefix });
      if (facts) return facts;
    } catch { /* nästa form */ }
  }
  return null;
