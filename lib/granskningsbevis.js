...
/** Dagens datum som ÅÅÅÅ-MM-DD i UTC — brytbar i test via `GRANSKNING_IDAG`. */
export function idag() {
  return process.env.GRANSKNING_IDAG || new Date().toISOString().slice(0, 10);
}

/** Är strängen ett datum som faktiskt finns? `2026-13-45` är siffror, inte en dag. */
export function arVerkligtDatum(text) {
  const m = /^(\d{4})-(\d{2})-(\d{2})$/.exec(String(text ?? ''));
  if (!m) return false;
  const [, ar, man, dag] = m.map(Number);
  const d = new Date(Date.UTC(ar, man - 1, dag));
  return d.getUTCFullYear() === ar && d.getUTCMonth() === man - 1 && d.getUTCDate() === dag;
}
...
export function parsaBevis(text) {
  ...
  // ⚠️ ETT FRAMTIDA DATUM UPPHÄVDE HELA TIEBREAKEN (granskarens [VAKT] 4). `BLOCKERAR 2026-09-13`
  // plus ett egenhändigt `MERGAS 2099-01-01` gav `ok: true` — ett dygn räckte för att neutralisera
  // regeln koden själv kallade «det enda ärliga». Och `2026-13-45` godtogs, eftersom regexen bara
  // räknade siffror. Ett datum som inte kan ha inträffat ordnar ingenting.
  if (!arVerkligtDatum(datumRad[1])) {
    return { giltigt: false, commits: [], dom, datum: null, skal: `datum «${datumRad[1]}» är inget verkligt datum` };
  }
  if (datumRad[1] > idag()) {
    return { giltigt: false, commits: [], dom, datum: null, skal: `datum «${datumRad[1]}» ligger i framtiden — en granskning som inte har skett kan inte väga tyngst` };
  }

  // ALLA `commits:`-rader läses, inte bara den första (granskarens [VAKT] 6): en lång lista
  // radbryts naturligt, och ett oförklarligt nej på en korrekt rapport är precis det som får en
  // vakt kringgången med --no-verify.
  const commitRader = [...huvud.matchAll(/^\s*commits?\s*:\s*(.+)$/gim)].map((m) => m[1]);
  if (commitRader.length === 0) return { giltigt: false, commits: [], dom, datum: datumRad[1], skal: 'rubriken saknar `commits:`' };
  const commits = commitRader.flatMap((r) => r.match(SHA) ?? []).map((s) => s.toLowerCase());
  if (commits.length === 0) {
