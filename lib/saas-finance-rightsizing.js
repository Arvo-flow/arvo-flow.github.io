...
function rightsize(lineItems, cfg) {
  ...
  return {
    ...
    needsReview: true,
    reviewPrompt: `Ni betalar för ${cfg.label} ${current.name} (${current.monthly} kr/mån). Nivån under, ${targetName}, kostar ${targetMonthly} kr/mån — ${deltaMonthly} kr/mån billigare. Ryms er användning (moduler, antal användare, verifikationsvolym) i ${targetName}? Bekräfta så realiserar vi ${fmtNumber(annualSaving)} kr/år.`,
