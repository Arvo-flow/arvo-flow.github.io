...
// ⚠️ TUSENTALSAVGRÄNSAREN SAKNADES I PROSAN (2026-09-22, avläst i DOM:en). Domen skrev
// «realiserar vi 2640 kr/år» medan kortet i samma vy skrev «2 640 kr/år» — samma tal, två
// skrivsätt, tre centimeter isär. `fmtNumber` i lib/format.js är backendens enda sv-SE-källa
// (regel 1); den anropas nu i stället för att talet interpoleras rått.
function rightsize(lineItems, cfg) {
  ...
  return {
    ...
    needsReview: true,
    reviewPrompt: `Ni betalar för ${cfg.label} ${current.name} (listpris ${current.monthly} kr/mån). Nivån under, ${targetName}, har listpris ${targetMonthly} kr/mån — ${deltaMonthly} kr/mån billigare. Ryms er användning (moduler, antal användare, verifikationsvolym) i ${targetName}? Bekräfta så realiserar vi ${fmtNumber(annualSaving)} kr/år.`,
    note: `Verifierad prisskillnad ${current.name}→${targetName}: ${deltaMonthly} kr/mån × 12 = ${fmtNumber(annualSaving)} kr/år (publikt listpris, ${cfg.urlText}). Förutsätter att behovet ryms i ${targetName}.`,
  };
}
