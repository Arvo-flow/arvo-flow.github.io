...
export function byggPrisunderlag({ annualCost, seats, ankare, niva } = {}) {
  ...
  // ── PER ENHET MÅSTE VARA ETT PRIS (granskning 2026-08-20) ────────────────────────────────
  // Med ett orimligt antal enheter (extraktionen läser ett belopp som en kvantitet) avrundas
  // priset per enhet till 0, och kortet hade visat "Ni betalar 0 kr per användare/år · −100 %"
  // utan score. Ett fail-open där allt annat i kedjan är fail-closed.
  // Ett bråktal duger heller inte: en licensmängd är ett heltal, och 0,5 platser gav +5 579 %.
  if (!(perEnhet > 0) || !Number.isInteger(enheter)) return null;
  ...
  // Kategorin kräver bekräftad nivå men vi har ingen: visa fakta, hävda inget avstånd.
  // Talen (vad ni betalar, vad billigaste jämförbara kostar) är sanna och nyttiga; PÅSTÅENDET
  // om hur långt ifrån kunden ligger är det som inte går att belägga utan produktnivån.
  const ovissNiva = kraverNiva && !(nivaGolvet > 0);
  ...
  return {
    ...
    // true = vi VET vad kunden betalar och vad billigaste jämförbara kostar, men inte om det är
    // samma produkt — så avstånd och score uteblir med avsikt.
    ovissNiva,
    ...
    underGolv: ovissNiva ? null : perEnhet <= golv,
    avstandPct: ovissNiva ? null : avstandPct,
...
export function scoreUrUnderlag(u) {
  ...
  // Vet vi inte vilken produkt kunden har, i en kategori där produktvalet avgör talet, finns
  // ingen jämförelse att betygsätta. Ett score här hade mätt produktval, inte pris.
  if (u.ovissNiva) return null;
