...
export async function markPending() {
  const kv = getKv();
  if (!kv) return;
  try { await kv.set(PENDING_KEY, 1, { ex: 86400 }); } catch { /* flaggan är en optimering, aldrig sanningen */ }
}
...
export function utfallFranSvar(a) {
  ...
  if (route === 'auto' || route === 'monitoring') return `${bas}${a.analysisId ? '·lagrad' : '·EJ_LAGRAD'}`;
  return bas;
}
