...
async function ensureTable(db) {
  ...
  // RUMMETS ADRESS (2026-09-24): ett jobb bär sin identitet. `fingerprint` är `adress:<nyckel>` när
  // fakturan kom till en rumsadress (lib/inkorgsadress.js) — då är avsändaren bara avsändare, aldrig
  // identiteten. NULL = gamla vägen, identiteten härleds ur `sender`.
  await db`ALTER TABLE ingest_jobs ADD COLUMN IF NOT EXISTS fingerprint TEXT`;
  await db`ALTER TABLE ingest_jobs ADD COLUMN IF NOT EXISTS agare_epost TEXT`;
}
...
export async function markPending() {
  ...
  //
  // ⚠️ OCH ANROPAREN MÅSTE KUNNA FRÅGA (2026-09-23). Varningen ovan skrevs ut — och på raden
  // efter skrev koa-om-alla «✓ … OCH köflaggan satt — drainen väcks nu», eftersom funktionen
  // returnerade `undefined` i båda fallen. Loggen sa emot sig själv på två rader i följd (Actions
  // 35822150557). Nu svarar funktionen: `true` = flaggan sattes, `false` = den sattes inte.
  if (!kv) {
  ...
  try {
    await kv.set(PENDING_KEY, 1, { ex: 86400 });
    return true;
  } catch (err) {
...
export async function enqueueJobs(jobs = [], { db: dbIn = null } = {}) {
  const db = dbIn ?? getDb();
  ...
  try {
    ...
    for (const j of jobs) {
      ...
      const rows = await db`
        INSERT INTO ingest_jobs (email_id, sender, filename, attachment_index, status, fingerprint, agare_epost)
        VALUES (${j.emailId}, ${j.sender}, ${j.filename ?? null}, ${j.attachmentIndex ?? 0}, 'pending',
                ${j.fingerprint ?? null}, ${j.agareEpost ?? null})
...
export async function claimBatch(n = 6, { db: dbIn = null } = {}) {
  const db = dbIn ?? getDb();
  // Ingen databas = vi vet ingenting om kön (samma sats som catch-grenen nedan). Att svara `[]`
  // vore att påstå att den är tom, och drainen hade släckt köflaggan på ett konfigurationsfel.
  if (!db) return null;
  try {
    await ensureTable(db);
    const rows = await db`
      ...
      RETURNING j.id, j.email_id, j.sender, j.filename, j.attachment_index, j.attempts, j.fingerprint, j.agare_epost
    `;
    // fingerprint + agareEpost bär rumsadressens identitet till drainen (jobbIdentitet). Utan dem föll
    // varje bulkjobb till avsändarens rum — kedjan enqueue → claimBatch → jobbIdentitet prövas i IA-11.
    return rows.map((r) => ({
      id: r.id, emailId: r.email_id, sender: r.sender,
      filename: r.filename, attachmentIndex: r.attachment_index, attempts: r.attempts,
      fingerprint: r.fingerprint ?? null, agareEpost: r.agare_epost ?? null,
    }));
  } catch (err) {
    // ── OKÄNT ÄR INTE SAMMA SAK SOM TOMT — ÄVEN HÄR (2026-08-24) ──────────────────────────────
    // Raden löd `return []`. Modulen bär regeln i klartext femtio rader upp och tillämpar den
    // korrekt på köflaggan (`null` = vet ej) — och bröt den här, i samma fil. `[]` betydde både
    // «kön är tom» och «jag kunde inte fråga», och drainen läser det som det förra: den bryter
    // loopen, ser claimed === 0 och RADERAR köflaggan — kundens enda signal om att arbete finns.
    // Konsekvensen är destruktiv, inte bara en missad tick: en transient DB-glitch under EN
    // körning skickar de följande ~14 cron-minuterna förbi Postgres helt. Kön är inte förlorad
    // (säkerhetsslotten plockar upp den) men latensen går från sekunder till en kvart, osynligt.
    // `null` = «jag vet inte». Anroparen MÅSTE skilja dem åt; drainen gör det nu.
...
export async function pendingCountBySender(sender) {
  ...
  try {
    const [r] = await db`SELECT COUNT(*)::int AS n FROM ingest_jobs WHERE sender=${sender} AND fingerprint IS NULL AND status IN ('pending','processing')`;
    return r?.n ?? 0;
  } catch { return 0; }
}
...
export async function failedCountBySender(sender) {
  ...
  if (!db || !sender) return 0;
  try {
    const [r] = await db`SELECT COUNT(*)::int AS n FROM ingest_jobs WHERE sender=${sender} AND fingerprint IS NULL AND status='failed'`;
    return r?.n ?? 0;
  } catch { return 0; }
}
...
export async function failedFilesBySender(sender, { limit = 25 } = {}) {
  ...
  // BUNDEN DEGRADERING, uttalad (2026-08-24): ett fel här ger `[]` — inga NAMN. Det kan aldrig
  // bli ett falskt «allt klart», för ANTALET kommer ur en separat fråga (failedCountBySender) och
  // rummet visar namnlistan först när antalet > 0. Kunden ser alltså «N misslyckades» utan namn,
  // aldrig «inga misslyckades». Skiljelinjen mot claimBatch: där var `[]` ett PÅSTÅENDE om kön.
  } catch (err) { console.error('[ingest-queue] failedFilesBySender:', err.message); return []; }
}

/**
 * JOBBETS IDENTITET (2026-09-24). Ett jobb till en rumsadress bär `fingerprint` (adress:<nyckel>) och
 * `agareEpost` (claimBatch:s form); då är avsändaren inte identiteten. Utan fingerprint gäller den gamla vägen: avsändaren.
 * Ren funktion — drainen och testerna frågar samma sak.
 */
export function jobbIdentitet(job, sha16) {
  // Formen är claimBatch:s (camelCase). Radens råstavning smäller i stället för att tyst ge «ingen ägare»
  // — ett okänt fältnamn är omöjligt att skilja från ett tomt fält (bibeln 24 aug, IA-11).
  if (job && Object.prototype.hasOwnProperty.call(job, 'agare_epost')) throw new Error('jobbIdentitet: agare_epost är radens stavning — läs claimBatch:s agareEpost');
  if (job?.fingerprint) {
    const e = typeof job.agareEpost === 'string' && job.agareEpost.includes('@') ? job.agareEpost : null;
    return { fingerprint: job.fingerprint, email: e, userEmail: e };
  }
  return { fingerprint: `mail:${sha16(job.sender)}`, email: job.sender, userEmail: job.sender };
}

/**
 * INTAGSFLÖDET (telemetrin i rummet, 2026-09-24): de senaste jobben för ett rum — per avsändaradress
 * (gamla vägen) och/eller per rumsadress. Kastar vid databasfel: ett flöde som inte kunde läsas och ett
 * tomt flöde får inte se likadana ut.
 */
export async function intagsflode({ sender = null, fingerprint = null, timmar = 24, db: dbIn = null } = {}) {
  const db = dbIn ?? getDb();
  if (!db) throw new Error('intagsflöde: ingen databas');
  if (!sender && !fingerprint) return [];
  return db`
    SELECT id, filename, status, outcome, error, created_at, done_at
    FROM ingest_jobs
    WHERE ((${sender}::text IS NOT NULL AND sender = ${sender} AND fingerprint IS NULL)
        OR (${fingerprint}::text IS NOT NULL AND fingerprint = ${fingerprint}))
      AND created_at > NOW() - make_interval(hours => ${timmar})
    ORDER BY created_at DESC LIMIT 60
  `;
}

/**
 * AVVISADE MEN BOKFÖRDA (2026-09-25). Ett mejl till en rumsadress över dagsgränsen analyseras inte — men det
 * får aldrig försvinna spårlöst: en adress utan ägare har ingen att mejla varningen till. Varje PDF bokförs
 * som ett fallet jobb (attempts = taket, så drainen rör det inte) och syns i rummet, där «Försök igen» kör
 * om det ur Resend (IA-13). Returnerar antalet bokförda, eller null om databasen inte kunde skriva.
 */
/** Skälet ett avvisat jobb bokförs med — rummet skiljer det från ett tekniskt fel (IA-15). */
export const DAGSGRANS_SKAL = 'dagsgransen_nadd';

export async function bokforAvvisade(jobs = [], skal, { db: dbIn = null } = {}) {
  const db = dbIn ?? getDb();
  if (!db) return null;
  try {
    await ensureTable(db);
    ...
    for (const j of jobs) {
      const rows = await db`
        INSERT INTO ingest_jobs (email_id, sender, filename, attachment_index, status, attempts, error, fingerprint, agare_epost)
        VALUES (${j.emailId}, ${j.sender}, ${j.filename ?? null}, ${j.attachmentIndex ?? 0}, 'failed', ${MAX_ATTEMPTS},
                ${String(skal).slice(0, 500)}, ${j.fingerprint ?? null}, ${j.agareEpost ?? null})
        ON CONFLICT (email_id, attachment_index) DO NOTHING
        RETURNING id
      `;
      if (rows.length) n++;
    ...
    console.error('[ingest-queue] bokforAvvisade:', err.message);
    return null;
  }
}

/** «Försök igen» för en rumsadress: dess fallna jobb → pending. Syskonet till retryFailedBySender (IA-13). */
export async function retryFailedByFingerprint(fingerprint, { db: dbIn = null } = {}) {
  const db = dbIn ?? getDb();
  if (!db || !fingerprint) return 0;
  try {
    const rows = await db`
      ...
      RETURNING id`;
    if (rows.length) await markPending();
    return rows.length;
  } catch (err) {
    console.error('[ingest-queue] retryFailedByFingerprint:', err.message);
    return 0;
  }
}
