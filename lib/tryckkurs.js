...
/** Ett tal är en kurs först när pappret SÄGER att det är en kurs, OMEDELBART före uttrycket. */
// ⚠️ `$`-ANKARET GÖR ARBETET — INTE FÖNSTRET (mätt 2026-09-10, fientlig granskning).
// Granskaren sabotage:ade fönstret 28 → 200 och fällde noll test. Skälet är att regexen är ankrad
// i SLUTET av utsnittet: mellan kursordet och talet får bara blanksteg, kolon eller likhetstecken
// stå. Ett kursord 150 tecken bort kvalificerar därför aldrig, oavsett hur stort fönstret är
// (körbart: `lasTryckKurs('kurs för april: ' + 'x'.repeat(150) + ' 10,42 SEK/USD', 'USD')` → null).
// Konstanten är alltså ingen tröskel utan bara en ÖVRE GRÄNS för hur mycket text regexen behöver
// se — den längsta frasen är «omräkningskurs: » på 16 tecken. Att kalla den ett «fönster» hade
// varit ett namn som lovar en mekanik den inte har, precis som `(?<!\d)` som togs bort i samma
// modul en timme tidigare. Invarianten som BÄR prövas av TK-15.
const KURSORD = /(?:väx(?:el|lings)kurs|omräkningskurs|valutakurs|kurs|exchange\s+rate|fx[\s-]?rate)\s*[:=]?\s*$/i;
const KURSORD_MAXLANGD = 28;
...
export function lasTryckKurs(text, valuta) {
  ...
  for (const re of monster) {
    for (const m of flode.matchAll(re)) {
      ...
      if (!KURSORD.test(flode.slice(Math.max(0, i - KURSORD_MAXLANGD), i))) continue;
