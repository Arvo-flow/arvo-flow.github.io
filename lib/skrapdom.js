...
const KVALIFICERARE = Object.freeze([
  ...
  [/bindning|bindningstid|vid\s+\d+\s*mån|\d+\s*mån(?:aders)?\s*avtal|årsavtal|uppsägningstid|årsvis|årsbetalning/i,
    'bindningsvillkor'],
  [/per\s+(?:användare|användaren|anv\.|anställd|anställda|medarbetare|deltagare|person|profil|postlåda|brevlåda|mailbox|plats|enhet|konto|user|seat|licens|mottagare)|\/\s*(?:användare|user|anv|person|plats|medarbetare)\b/i,
    'enhetskvalificerare'],
]);
 ...
 * klassningen nedanför, där varje träff får ett namngivet utfall.
 *
 * ⚠️ MELLANRUMSKLASSEN VAR EN UPPRÄKNING, OCH UPPRÄKNINGEN VAR FEL (granskningen 2026-09-16).
 * `[^\S\r\n]` är «\s utom radbrytning» — och JS:s \s innehåller inte formatkontrolltecknen.
 * `Pro 1<U+200B>299 kr/mån` gav därför fortfarande **299**, tyst, liksom U+2060, U+00AD, U+200C,
 * U+200D, U+180E, U+0085 och U+0000. Vitlistan fick aldrig frågan: matchningen vid «1» föll, och
 * mönstret startade om på «299». Hålet satt UPPSTRÖMS om den grind som skulle stänga det.
 *
 * Min commit påstod «åtta blankstegstecken jag aldrig tänkte på täcks utan att nämnas». Mätt: sex.
 * Klassen är nu deklarerad ur Unicodes kategorier i stället för uppräknad — då finns inget «jag
 * tänkte inte på» kvar att mäta.
 */
// Allt som kan stå MELLAN två siffergrupper utan att synas. Klassen är DEKLARERAD ur Unicodes
// egna kategorier (mellanrum · format · kontroll) minus radbrytningarna, aldrig uppräknad —
// det var uppräkningen som var felet, och en uppräkning av tio tecken i ett TEST är samma fel
// ett lager upp. RegExp-flaggan v ger mängdsubtraktion, så radbrytningens undantag blir en rad.
const OSYNLIGT = '[[\\p{Zs}\\p{Cf}\\p{Cc}]--[\\r\\n]]';
const KANDIDAT = new RegExp(
  '([\\u002d\\u2212\\u2013])?' + OSYNLIGT + '*'
  + '(\\d(?:[\\d.,]|' + OSYNLIGT + ')*\\d|\\d)' + OSYNLIGT + '*'
  + '(?::-|kr\\.?|kronor|sek)' + OSYNLIGT + '*(?:/|per' + OSYNLIGT + '+)' + OSYNLIGT + '*'
  + '([\\p{L}\\u00b2][\\p{L}\\u00b2.]*(?:' + OSYNLIGT + '*/' + OSYNLIGT + '*[\\p{L}\\u00b2][\\p{L}\\u00b2.]*)*)',
  'giv');
 ...
 * enhet. En tyst bortsortering är precis hur det falska golvet uppstod, två gånger, i två olika
 * kostymer.
 *
 * ⚠️ MEN DEN FJÄRDE UTGÅNGEN FINNS, och att förneka den vore samma fel igen. Ett prisliknande
 * uttryck som `KANDIDAT` inte matchar ALLS blir varken pris, avvisat eller `ejManad` — det syns
 * inte. Granskningen mätte formen: `innerText` lägger ibland en RADBRYTNING mellan talet och
 * enheten, och den är utesluten med avsikt (den skiljer «M365 E3\n490 kr/mån» åt). Mätt på
 * oderland-sidan: 25 råa förekomster av «kr», 0 redovisade. `499:-` och `kr.` var två andra och
 * är nu lagade.
 *
 * Riktningen på den luckan är den säkra: ett TAPPAT pris kan bara höja `lagsta`, alltså tona ner
 * en besparing. Ett FALSKT lågt pris överdriver den. Att de tre utgångarna är redovisade är
 * därför värt mer än att de vore uttömmande — men de är inte uttömmande, och det ska stå här.
 *
