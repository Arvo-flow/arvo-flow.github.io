...
// Zontilldelning via leverantörsnamn (regex → SE1–SE4).
// Täcker de vanligaste leverantörerna i respektive region.
const ZONE_PATTERNS = [
  { zone: 'SE1', pattern: /lule(å|a)|vattenfall.*norr|skellefteå/i },
  { zone: 'SE2', pattern: /sundsvall|öst.*norrland|jämtkraft/i },
  { zone: 'SE3', pattern: /stockholm|stockholm.*elhandel|e\.on|vattenfall|mälarenergi|tekniska.*verken/i },
  { zone: 'SE4', pattern: /malmö|sydsvenska|öresundskraft|kraftringen|skånska/i },
];
 ...
 *
 * @param {object} params
 * @param {number} params.annualKwh         - uppskattad årsförbrukning kWh
 ...
 * @returns {Promise<object>} { zone, spot, ranked, best, current, saving, savingPct }
 */
export async function getElIntelligence({ annualKwh, currentPriceKwh, supplierName }) {
