...
export const VILLKORSBOK = {
  bahnhof: {
    ...
    // alltså larma här, och posten är därför inte ovaktbar.
    // DISTRIBUTIONSPUNKTEN BEVISAD 2026-08-10 av scripts/probe-villkorssida.mjs: sonden gick från
    // bahnhof.se, följde varje länk som nämner villkor, och fann att just den här sidan bär exakt
    // vår fastnålade PDF bland sina 7 PDF-länkar. Aldrig en gissad adress — beviset är binärt.
    // (Observera värdskiftet: PDF:en ligger på www.bahnhof.se, villkorssidan på apex-domänen.)
    villkorssida: 'https://bahnhof.se/villkor/',
    // Bahnhofs PDF finns INTE i sondens textdump (0 träffar i ops/probe-avtal-corpus.txt), så det
    // finns ingen bevisat extraherad text att kurera en kontrollfras ur. Fältet lämnas null — att
    // hitta på en fras vore precis den gissning hela läsaren finns för att omöjliggöra. Läsaren
    // rapporterar OLÄSBAR och skriver ut ett smakprov ur den verkliga texten; kontrollfrasen
    // kureras ur DET smakprovet, i en andra vända.
    // Kurerad 2026-08-09 ORDAGRANT ur skriptets egen verifierade extraktion (4 sidor, 18 536
    // tecken) — aldrig ur minnet. Medvetet vald ur dokumentets titelrad, där varje å/ä/ö kom ut
    // korrekt: samma textlager tappade ett ä längre ned ("Dessa villkor galler"), så frasen
    // provar just den svagheten och bevisar att teckenkodningen bär där citatet ska läsas.
    kontrollfras: 'Allmänna villkor Tjänster för företag, bostadsrättsföreningar & fastighetsägare Bahnhof AB',
    citat: 'Uppsägningstid är tre (3) månader. Efter Avtalstiden förlängs Avtalstiden automatiskt löpande med tre (3) månader.',
    verifierad: '2026-08-10',
    // FÖRSEGLAD 2026-08-10 av scripts/forsegla-villkor.mjs: citatet lästes ordagrant ur exakt
    // dessa bytes. Läsning och ankare i samma sekund — därför samma datum.
    dokumentSha256: 'be2c89f54092a812c6e101357942051b78cea238ba80136610d4db18ba6765dc',
    forsegladDatum: '2026-08-10',
  },
  telia: {
    supplier: 'Telia',
    ...
    // KONTROLLFRASEN (lib/villkorslasare.js): en andra ordagrann sträng ur SAMMA dokument, som
    // bevisar att textlagret gick att läsa. Utan den vore "citatet saknas" ett antagande i stället
    // för ett fynd — ett trasigt textlager ger mycket text där ingenting hittas.
    // Hämtad ORDAGRANT ur sondens verkliga extraktion (ops/probe-avtal-corpus.txt 2026-07-03),
    // aldrig ur minnet: rubriken till just det avsnitt vårt citat bor i.
    kontrollfras: '19 Uppsägning av Avtalet 19.1 Kunden har rätt att med omedelbar verkan säga upp den del av Avtalet som avser en Tjänst',
    verifierad: '2026-08-10',
    // FÖRSEGLAD 2026-08-10 av scripts/forsegla-villkor.mjs: citatet lästes ordagrant ur exakt
    // dessa bytes. Läsning och ankare i samma sekund — därför samma datum.
    dokumentSha256: '17e28faf33ceac1251b6ec82a751f54b4ad2f9a3f54d632dcdd7ab3dbe1e35d7',
    forsegladDatum: '2026-08-10',
  },
};
...
/**
 * Sista dagen att säga upp inför `slut` — EN definition för både avtalsvyn och fakturaklockan
 * (lib/contract-clock.js). Månader räknas i KALENDERN, dagar i dygn: «3 månader» före 1 januari är
 * 1 oktober, inte 3 oktober. Fakturaklockan räknade förut 90 dygn och angav sista dagen upp till två
 * dagar för SENT (mätt 2026-09-23, AK-02).
 * @param {Date} slut  UTC-midnatt för avtalets slutdatum
 * @returns {Date|null} UTC-midnatt, eller null utan giltig uppsägningstid
 */
export function sistaUppsagningsdag(slut, regler) {
  if (!(slut instanceof Date) || Number.isNaN(slut.getTime())) return null;
  const s = noticeSubtractor(regler ?? {});
  return s ? s.subtractFrom(slut) : null;
}
