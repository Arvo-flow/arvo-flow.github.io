...
export function buildAvtalView(terms = {}, { today = new Date() } = {}) {
  const { fields, rules = null, citat = null, supplier = null, readAt = null } = terms;
  if (!fields?.avtalsstart) return null;
  const clock = computeContractOutcome(fields, rules ?? {}, { today });
  ...
  return { supplier, readAt, fields, rules, citat, clock, nastaPeriodSlut, bindningLabel, uppsagningLabel, forlangningLabel };
}
