...
export default {
  ...
  bevakadeTiers: PRODUKTER.flatMap((p) => p.tiers.map((t) => t.key)),
  // VAKTKONTRAKTET (lib/vaktkontrakt.js): den fråga som ska ställas innan något kallas vakt.
  fangar: 'Atlassian ändrar sitt publika USD-listpris för Jira eller Confluence vid det seat-antal vakten explicit ställer in.',
  blind: 'Prisförändringar i trappan vid ANDRA seat-antal än det inställda — Atlassian-läxan 2026-08-05: samma produkt visar olika pris beroende på väljaren.',
  needsBrowser: true,
  schedule: '0 5 * * 1',
  async run() {
    ...
    // NOLL CHECKAR ÄR RÖTT, INTE VÄNTANDE (rättat 2026-08-20). Raden returnerade tidigare
    // `{ skipped: true }`, vilket gjorde fabriken grön när sidan bytt form och vi läst ingenting.
    // Det är precis det tillstånd `!(res.checks?.length)` i scripts/verify.mjs finns för att fånga.
