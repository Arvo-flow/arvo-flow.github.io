...
// Adobe skriver alla priser med två decimaler och komma: "985,00 SEK", "311,25 SEK". Matcha exakt det
// (heltalsankare som 985 visas som "985,00" → integer-regex utan decimaler missar dem).
const fmtSek = (n) => new Intl.NumberFormat('sv-SE', { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(n);
const has = (text, n) => text.includes(`${fmtSek(n)} SEK`);
...
export default {
  ...
  bevakadKategori: 'saas-creative',
  // VAKTKONTRAKTET (lib/vaktkontrakt.js): den fråga som ska ställas innan något kallas vakt.
  fangar: 'Adobe ändrar sitt publika SEK-listpris på den bot-skyddade prissidan som vakten når via stealth.',
  blind: 'Priser bakom inloggning eller regionala varianter av sidan — vakten ser den publika svenska vyn som en utloggad besökare får den.',
  needsBrowser: true,
  needsStealth: true,
  schedule: '15 5 * * 1',
  async run() {
