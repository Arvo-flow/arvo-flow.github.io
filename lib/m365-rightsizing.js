...
export function m365Rightsizing(currentTierKey, seats) {
  ...
  return {
    ...
    needsReview:          true,
    reviewPrompt:
      `Ni betalar för ${LABELS[currentTierKey]} (listpris ${fmt2(cur.msrpAnnual)} kr/användare/månad) — full enterprise-svit ` +
      `med ${ENTERPRISE_FEATURES[currentTierKey]}. Det är sällan motiverat under ~100 användare. ` +
      `Microsoft 365 Business Premium (listpris ${fmt2(tgt.msrpAnnual)} kr/användare/månad) ger Intune MDM + Defender for Business ` +
