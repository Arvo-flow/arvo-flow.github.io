...
export function runIntegrityChecks(extracted, invoiceHeader = '') {
  ...
  try {
    ...
    //
    // RÄTTELSE 2026-08-24: här stod `monthsBetween(extracted.billingPeriod) ?? 1`. `billingPeriod`
    // är efter aggregeringen ett ENUM, och monthsBetween kunde inte parsa någon indata alls — så
    // `?? 1` gav multiplikatorn 12 för VARJE faktura, och kontrollen "korrigerade" varje kvartals-
    // och årsfaktura (×3 respektive ×12 fel). Kundens tal rördes aldrig, men varje sådan rad gick
    // till labeled_corrections och lärde flywheeln ett mönster ur en bugg som inte fanns.
    //
    // Nu: samma tabell som aggregeringen räknade med (lib/faktureringsperiod.js, regel 1), och
    // ett `null` som betyder «jag vet inte» — då dömer kontrollen INTE. En obestämd period är
    // inte ett skäl att anta månadsvis; det var precis det antagandet som var felet.
    const perAr = perioderPerAr(extracted.billingPeriod);
    if (result.recurring && extracted.recurringAmount > 0 && extracted.annualCost > 0 && perAr > 0) {
      const expected   = extracted.recurringAmount * perAr;
