...
/** Prosa-artefakter: domar, bibeln, arbetsordrar. Vakten DÖMER dem inte — den RÄKNAR dem. */
export const PROSA = /\.mde?$|^ops\/|^CLAUDE\.md$/i;

/**
 * VAKTENS EGEN BLINDFLÄCK, GJORD SYNLIG (2026-09-20).
 *
 * Grundardoktrin: «avläsning framför slutsats». Den kom ur att en dom jag mergat — och som
 * handlade om just den felformen — avslutades med ett påstående om databasen som var en SLUTSATS,
 * inte en avläsning, och som visade sig FALSKT när det mättes dagen därpå.
 *
 * Mätt 2026-09-20 varför vakten inte fångade det:
 *   · samma påstående i `lib/x.js` → 1 fälld. I `ops/GRANSKNING-x.md` → **0 fällda**.
 *     `BEVAKADE` täcker kod; domarna och bibeln låg utanför.
 *   · att bara vidga `BEVAKADE` till prosa är MÄTT OGÖRLIGT: ordlistan träffar **127 rader** i
 *     `ops/*.md` + `CLAUDE.md`, varav **98 saknar test-ID**. Även med ett mjukare beviskrav
 *     («mätt», ett skriptnamn, ett datum) återstår **80 falsklarm**. En vakt som fäller åttio
 *     korrekta historiska rader blir avstängd inom en dag — bibelns egen mätning av ordet
 *     «aldrig» (25 träffar på 20 commits) säger exakt detsamma.
 *
 * Alltså: vakten kan INTE döma prosa, och att låtsas annat vore ett påstående utan mätning i
 * vakten mot påståenden utan mätning. Men den kan sluta se ut som om den läst allt. Funktionen
 * RÄKNAR prosapåståendena i diffen så att skalet kan skriva ut dem — ett grönt som betyder «jag
 * tittade inte» är farligare än ett rött. PV-16.
 *
 * @returns {Array<{ fil: string, nr: number, rad: string, ord: string }>}
 */
export function prosaPastaenden(diff) {
  const ut = [];
  for (const [fil, rader] of tillagdaRader(diff)) {
    if (BEVAKADE.test(fil) || !PROSA.test(fil)) continue;
    for (const { rad, nr } of rader) {
      const ord = PASTAENDEORD.find((o) => rad.toLowerCase().includes(o));
      if (ord) ut.push({ fil, nr, rad: rad.trim().slice(0, 96), ord });
    }
  }
  return ut;
}
