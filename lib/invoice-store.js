...
import { stamplaAnalys } from './analysstampel.js';
import { valjRattstorlek } from './rattstorleksfynd.js';
...
export async function storeTriaged({ fingerprint, pdfHash, supplier, category, route, reason, userEmail, invoiceNumber = null, lineItems = null, db: dbIn = null }) {
  ...
  try {
    ...
    await db`
      ...
      ON CONFLICT (fingerprint, pdf_hash) DO UPDATE
        SET route = EXCLUDED.route, triage_reason = EXCLUDED.triage_reason,
            ...
            should_switch = false,
            -- ⚠️ KATEGORIN FRÖS HÄR (mätt 2026-09-23, omkörningen av grundarens 25-bunt). Satsen
            -- uppdaterade route och skäl men ALDRIG kategorin — så en faktura som en gång
            -- triagerats som 'uncategorized' förblev det, även när en ny körning KÄNDE IGEN den
            -- (Atlassian, Dustin, Komplett, Systemair, Ahlsell: nytt skäl no_benchmark, med
            -- riktig kategori i anropet, gammal «okategoriserad» kvar i raden). Samma frusna dom
            -- som valutafallet, i ett fält stämpeln inte täcker.
            -- En BÄTTRE läsning får ersätta en sämre, aldrig tvärtom (samma regel som supplier i
            -- storeAnalysis): 'uncategorized' — det värde som skrivs när kategorin är okänd — får
            -- aldrig radera en känd kategori. TR-01..03.
            category = CASE WHEN EXCLUDED.category = 'uncategorized' THEN invoice_analyses.category
    ...
    // Ett triage-BESLUT är också en dom av en viss kod — och det var just triagerade rader
    // (Ring 1 före valutafixen) som stod frusna sedan 9 sep utan att någon kunde se det.
    await stamplaAnalys(db, { fingerprintHash: hashedFp, pdfHash });
    // En triagerad faktura bär inget prissatt fynd — ett rätt-storleksfynd från en tidigare körning
    // får inte stå kvar bredvid «vi prissatte den inte». Egen sats, egen catch. RS-07.
    try {
      await db`UPDATE invoice_analyses SET rattstorlek_json = NULL
               WHERE fingerprint = ${hashedFp} AND pdf_hash = ${pdfHash}`;
    } catch (err) { console.error('[invoice-store] storeTriaged rattstorlek_json:', err.message); }
...
export async function storeAnalysis({
  ...
  try {
    ...
    // VILKEN KOD DÖMDE (2026-09-23): skrivs vid varje lagring, även en omkörning. Egen sats,
    // egen catch — se lib/analysstampel.js.
    await stamplaAnalys(db, { id });
    ...
    // RÄTT-STORLEKSFYNDET (2026-09-23): motorn räknade det, lagringen kastade det — rummet kunde
    // aldrig visa det (lib/rattstorleksfynd.js). Skrivs ALLTID, även som NULL: den nyaste körningen
    // äger hela sin dom, och ett kvarlämnat fynd bredvid en dom som inte bär det är en lögn i
    // rummet (samma skäl som lead_finding_json ovan). Egen sats, egen catch. RS-05, RS-06.
    if (id) {
      const rs = valjRattstorlek(recommendation);
      try {
        await db`UPDATE invoice_analyses SET rattstorlek_json = ${rs ? JSON.stringify(rs) : null}::jsonb WHERE id = ${id}`;
      } catch (err) { console.error('[invoice-store] rattstorlek_json:', err.message); }
    }
    ...
    // Lagras rått (datum), klockan beräknas färsk vid läsning så "dagar kvar" aldrig blir
    // inaktuell. servicePeriodEnd sätts av extract.js bara vid uttalad bindningstid (Zero Trust).
    //
    // ⚠️ `AND contract_end_date IS NULL` STOD HÄR (rättat 2026-09-23): den FÖRSTA läsningen vann för
    // alltid, så en omkörning med rättad extraktion kunde aldrig nå rummet eller påminnelserna.
    // Samma regel som fakturanumret: en bättre läsning får ersätta, en tom får aldrig radera (AK-10).
    if (id && extracted?.servicePeriodEnd) {
      try {
        await db`UPDATE invoice_analyses SET contract_end_date = ${extracted.servicePeriodEnd}::date WHERE id = ${id}`;
      } catch { /* kolumn ej migrerad än → klockan i rummet aktiveras efter migrering */ }
    }
    // UPPSÄGNINGSTIDEN (2026-09-23): lagrades aldrig — rummet och mejlen räknade därför mot slutdatum
    // och missade sista uppsägningsdag vid varje uppsägningstid över 30 dagar (mätt, AK-09). Lagras i
    // regelform ({uppsagningstidMan} ELLER {uppsagningstidDagar}), egen sats, egen catch.
    if (id && extracted?.uppsagning) {
      try {
        await db`UPDATE invoice_analyses SET uppsagning_json = ${JSON.stringify(extracted.uppsagning)}::jsonb WHERE id = ${id}`;
      } catch (err) { console.error('[invoice-store] uppsagning_json:', err.message); }
    }
    return id;
  } catch (err) {
...
export const VALFRIA_KOLUMNER = [
  ...
  ['line_items_json', 'JSONB'], ['rattstorlek_json', 'JSONB'], ['uppsagning_json', 'JSONB'],
  ...
  try {
    return await db`
      SELECT
        ...
        seat_count, price_per_seat_monthly, lead_finding_json, contract_end_date, health_score, triage_reason, contract_terms_json, invoice_number, line_items_json, rattstorlek_json, uppsagning_json, user_email
