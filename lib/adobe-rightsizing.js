...
export function adobeRightsizing(lineItems, seats) {
  ...
  return {
    ...
    needsReview:         true,
    reviewPrompt:
      `Ni betalar för Adobe ${LABELS['all-apps']} (listpris ${fmt2(currentEx)} ${unit} exkl moms, ${skuLabel}). ` +
      `Använder era användare i praktiken bara ETT program (t.ex. bara Photoshop)? Då räcker ` +
      `${LABELS['single-app']} (listpris ${fmt2(targetEx)} ${unit} exkl moms) — ${fmt2(perSeatDelta)} ${unit} billigare per licens. ` +
