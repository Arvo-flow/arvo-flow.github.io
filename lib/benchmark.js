...
export function getPublicListBenchmark({ category }) {
  const mock = getMockBenchmark({ category, industry: 'byraer', employees: 5 });
  ...
  return { ...mock, referensProdukt: BRANCHINDEX[category]?.referensProdukt ?? null, n: 0 };
}

export async function storeDatapoint({ category, supplier, annualCost, industry, employees, seatCount = null }) {
  const db = getDb();
  if (!db) return;
