...
import { arTestidentitet, TEST_EXAKTA, TEST_DOMAN, TEST_LOKALDELAR } from './test-surface.js';
...
/**
 * VALFRIA KOLUMNER SKRIVS I EGEN SATS MED EGEN CATCH (bibelns 11 september-regel).
 *
 * `per_user_monthly_exvat` och `tier` är Vallgravens fält — det som gör «marknaden betalar X för
 * T2» beräkningsbart. De låg oskrivna tills 2026-09-20: `buildTelekomDatapoint` producerade dem
 * men hade NOLL anropare, så varje molnväxelrad i prisboken bar NULL i båda.
 *
 * De får ALDRIG ligga i huvudinserten. Saknas kolumnen — glappet mellan deploy och migrering —
 * hade hela skrivningen kastat, `storeDatapoint` svalt felet och **kundens datapunkt aldrig
 * landat**. Det är pdf_hash-fallet 10 september ordagrant. Fail-open på fältet (VD-04), aldrig på raden.
 *
 * Returnerar inget: en misslyckad fältskrivning är ett loggat OKÄNT, aldrig ett kastat fel.
 */
async function skrivPerEnhet(db, id, perUserMonthlyExVat, tier) {
  if (!id || (perUserMonthlyExVat == null && tier == null)) return;
  try {
    await db`
      UPDATE invoice_datapoints            -- moat: per-enhet-fälten, testidentiteten redan utesluten ovan
      SET per_user_monthly_exvat = COALESCE(${perUserMonthlyExVat}, per_user_monthly_exvat),
          tier                   = COALESCE(${tier}, tier)
      WHERE id = ${id}
    `;
  } catch (err) {
    console.error('[benchmark] per-enhet-fälten skrevs INTE (raden finns kvar utan dem): '
      + err.message);
  }
}

export async function storeDatapoint({ category, supplier, annualCost, industry, employees, seatCount = null, segmentOkant = false, pdfHash = null, userEmail, perUserMonthlyExVat = null, tier = null, db: dbIn = null }) {
  ...
  try {
    ...
    try {
      const rader = await db`
        ...
        RETURNING id
      `;
      await skrivPerEnhet(db, rader?.[0]?.id, perUserMonthlyExVat, tier);
    } catch (err) {
      ...
      const reserv = await db`
        ...
        RETURNING id
      `;
      await skrivPerEnhet(db, reserv?.[0]?.id, perUserMonthlyExVat, tier);
    }
