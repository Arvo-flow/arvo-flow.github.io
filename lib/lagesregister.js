...
export function diagnosEtikett(score) {
  if (score == null || !Number.isFinite(Number(score))) return null;
  return score < 45 ? 'Kritisk' : score < 65 ? 'Suboptimerat' : score < 80 ? 'Förbättringsläge' : 'Optimalt';
}
...
export function fakturaLage(svar, { rattstorlekFalt = [] } = {}) {
  ...
  return { ...d, etikett: d.matt ? diagnosEtikett(d.score) : null, rubrik, harByte: rek.shouldSwitch === true && (Number(rek.netSaving) || 0) > 0 };
}
