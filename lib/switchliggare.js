...
export class PgStore {
  /** @param {{ db?: Function }} [opts]  db injiceras i tester; produktionen läser getDb(). */
  constructor({ db = null } = {}) { this._db = db; }
  _getDb() { return this._db ?? getDb(); }
  ...
  /**
   * KONTRAKTET ÄR FileStore:s: `list()` returnerar switch-ID:n, inte poster. Orkestratorns
   * `list()`, `findBySigningDocId()` (Scrive-webhooken) och `findDueScheduled()` (cron) itererar
   * id:n och anropar `load(id)`. Första versionen returnerade hela poster — «samma gränssnitt»
   * stod i huvudet men var aldrig prövat, och alla tre hade brutit den dag PgStore blev default.
   * (Fable 5.1:s granskning 2026-09-01. Låst av SL-01..03.)
   *
   * @returns {Promise<Array<string>|null>} null = OKÄND (ingen databas), aldrig tolkad som tom.
   */
  async list() {
    const db = this._getDb();
    if (!db) return null;
    await ensure(db);
    const rader = await db`SELECT id FROM switch_records ORDER BY created_at ASC`;
    return (rader ?? []).map((r) => r.id);
  }

  /** Hela posterna, för arvodeskörningen. Samma tillstånd som `list()`: null = OKÄND. */
  async listRecords() {
    const db = this._getDb();
    if (!db) return null;
    await ensure(db);
...
/** Bekvämlighet för körningen. Se `listRecords()` för tillstånden — `null` betyder OKÄND. */
export async function hamtaLiggare() {
  return new PgStore().listRecords();
}
