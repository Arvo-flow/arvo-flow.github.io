...
import { arMarknadshandelse } from './prisbaslinje.js';
 ...
 */
/**
 * Larmets hela dom: modellens svar (bedomLarmunderlag) OCH att ändringen är en marknadshändelse
 * (lib/prisbaslinje.js). Båda larmvägarna anropar DENNA — aldrig bedomLarmunderlag direkt (BL-05).
 */
export function bedomLarm(alert) {
  const u = bedomLarmunderlag(alert?.haiku);
  if (u.niva !== 'verifierad') return u;
  const h = arMarknadshandelse(alert);
  return h.ja ? u : { niva: 'obekraftad', skal: h.skal };
}
