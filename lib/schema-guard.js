 ...
 */
export function guardToolPayload(p) {
  try {
    // Destruktureringen bor INNE i try — i parameterlistan kastade `guardToolPayload(null)`
    // innan fail-open-nätet nedan ens fanns. Samma hål som judgeProjection bar (OB-15).
    const { agent, tool, payload, enforce } = p ?? {};
