...
export async function getCorrections({ limit = 100, category = null, field = null, correctedBy = null } = {}) {
  ...
  try {
    return await db`
      ...
      LEFT JOIN invoice_analyses ia ON ia.id = lc.analysis_id   -- liggare: internt: admins
      -- granskningsvy av korrektioner (enda anroparen är api/admin/corrections.mjs, ADMIN_TOKEN-
      -- grindad). Ingen kundyta läser raderna och inget tal härleds ur dem.
      --
      -- ⚠️ SKÄLET SA FÖRST «flywheelns few-shot-underlag». Det var FALSKT: getFewShotExamples
      -- längre ned läser labeled_corrections ensam och joinar aldrig liggaren. Klassen var rätt,
      -- skälet beskrev en annan mekanik än koden utför — exakt den form som gjorde
      -- smyghöjningssatsen till ett [KUND] i domen det här arbetet svarar på. Ett skäl ingen
      -- läser efter är ett skäl som blir osant utan att någon märker det.
...
export async function getPatterns() {
  ...
  try {
    return await db`
      ...
      -- KARANTÄN (2026-08-24): kontroll 5 skrev en falsk korrektion på varje kvartals-, års-
      -- och prorata-faktura (multiplikatorbuggen + självjämförelsen). Raderna är bevarade som
      -- historik — RADERING är grundarens beslut, inte kodens — men de får aldrig derivera
      -- regler: ett mönster lärt ur en bugg som inte finns är flywheelns motsats.
      WHERE reason NOT LIKE 'annual_cost_deviates%'
