...
export function switchArvode({
  ...
  // ⚠️ `new Date(null)` är 1970-01-01, INTE Invalid Date — och detsamma gäller `0` och `false`.
  // Ett odaterat byte blev därför «utfört för 20 000 dagar sedan, karensen passerad för länge
  // sedan → fakturera». Felfamiljen i sin farligaste riktning: «vi vet inte när arbetet gjordes»
  // lånade ett fullt giltigt datum och blev till en faktura. Bara en Date eller en icke-tom
  // sträng är en datering; allt annat är ett OKÄNT och håller arvodet. (AK-03/SA-09, 2026-08-30.)
  const utfort = arbeteUtfortAt instanceof Date
    ? arbeteUtfortAt
    : (typeof arbeteUtfortAt === 'string' && arbeteUtfortAt.trim()
        ? new Date(arbeteUtfortAt)
        : new Date(NaN));
