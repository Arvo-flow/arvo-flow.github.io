...
// ── DATUMET ÄR ÖVERRASKNINGEN (2026-09-05) ──────────────────────────────────────────────────
// «Tolv månader utöver planen» är ett abstrakt tal. «Avbetalningen var slutbetald i september
// 2025 — sedan dess har ni betalat 2 450 kr i månaden för en server ni redan ägde» är ett DATUM
// en ekonomichef minns, och som hen kan gå tillbaka och kontrollera i sin egen bokföring.
//
// Talet kommer ur kundens eget papper: fakturadatumet minus antalet månader utöver planen.
// Ingen marknadsdata, ingen prisbok, ingen modell — ren aritmetik på två avlästa fält.
//
// FAIL-CLOSED PÅ PÅSTÅENDET (FO-05): utan fakturadatum hävdas ingen månad. Fyndet står kvar med
// sitt tal och sitt antal månader; det är datumet som uteblir, aldrig fyndet.
// Dagen hävdas ALDRIG — bara månaden. Avbetalningsplaner löper på månadsgräns, och ett exakt
// datum vore en precision fakturan inte bär.
const MANADER = ['januari', 'februari', 'mars', 'april', 'maj', 'juni',
  'juli', 'augusti', 'september', 'oktober', 'november', 'december'];

export function slutbetaldManad(fakturadatum, manaderOver) {
  if (!Number.isFinite(manaderOver) || manaderOver <= 0) return null;
  const d = fakturadatum instanceof Date ? fakturadatum
    : (typeof fakturadatum === 'string' && fakturadatum.trim() ? new Date(fakturadatum) : null);
  if (!d || Number.isNaN(d.getTime())) return null;          // okänt datum → inget påstående
  // Dag 1 i UTC: annars kan «31 mars minus 1 månad» landa i mars igen (månadslängdsfällan).
  const m = new Date(Date.UTC(d.getUTCFullYear(), d.getUTCMonth() - manaderOver, 1));
  return { text: `${MANADER[m.getUTCMonth()]} ${m.getUTCFullYear()}`, iso: m.toISOString().slice(0, 7) };
}
...
const DETECTORS = [
  {
    ...
    derive: (d, monthly, fakturadatum) => {
      const p = parseAmortMonths(d);
      const months = p ? p.paid - p.total : 0;
      const slut = slutbetaldManad(fakturadatum, months);
      if (!(months > 0) || !Number.isFinite(monthly) || monthly <= 0) {
        return { monthsOverpaid: months > 0 ? months : 0, ...(slut ? { slutbetald: slut.text, slutbetaldIso: slut.iso } : {}) };
      }
      return {
        monthsOverpaid: months,
        overpaidToDate: Math.round(monthly * months),
        // Månadsbeloppet bärs eget så kortet kan visa vad varje ytterligare månad kostar utan
        // att räkna själv (regel 2: koden räknar, ytan renderar).
        manadsbelopp: Math.round(monthly),
        ...(slut ? { slutbetald: slut.text, slutbetaldIso: slut.iso } : {}),
      };
    },
    ...
    text: (d, monthly, fakturadatum) => {
      const p = parseAmortMonths(d);
      const months = p.paid - p.total;
      const slut = slutbetaldManad(fakturadatum, months);
      // ⚠️ MÅNADSBELOPPET SKREVS FÖRST UT HÄR — och det bröt regeln från 15 augusti: prosan
      // upprepar aldrig kortets nyckeltal, den förklarar VARFÖR de finns. Med kortets nya
      // framåtrad («2 450 kr/mån löper vidare») hade talet stått TRE gånger i samma kort, och i
      // en faktura med en enda månads överbetalning kolliderar det dessutom med kravet (FO-08).
      // Det gamla testet fällde mig direkt. Datumet är överraskningen; beloppen bär chipen.
      const nar = slut
        ? `Avbetalningen var slutbetald i ${slut.text}`
        : `Avbetalningen är slutbetald sedan ${versal(SKRIVNA[months] ?? months)} ${months === 1 ? 'månad' : 'månader'} tillbaka`;
      // ⚠️ «VI BEVAKAR ATT DEN FÖRSVINNER» STOD HÄR (borttaget 2026-09-06, grundarfråga).
      // Grep gav EN enda träff i hela kodbasen: strängen själv. Det fanns ingen bevakning —
      // ingen rad i contract_timelines, inget schemalagt återbesök, ingenting som kunde upptäcka
      // att posten stod kvar. Ett kundlöfte utan mekanik (regel 9), skrivet i den modul vars hela
      // uppgift är att bara säga det kundens egen rad bär.
      //
      // Ersatt av något vi FAKTISKT äger: mail-in-ingesten är live och verifierad end-to-end
      // sedan 11 juni. Det är en inbjudan till en mekanism i stället för ett löfte om en, och den
      // flyttar kontrollen dit där den kan utföras. FO-12 låser att löftet inte kan återinföras.
      return `Raden ovan står på månad ${p.paid} av ${p.total}. ${nar} — utrustningen är redan er, `
        + `och sedan dess har ni betalat för den varje månad. Posten ska bort från nästa faktura, `
        + `och varje månad den står kvar kostar lika mycket till. Skicka nästa faktura till oss så `
        + `kontrollerar vi att den är borta.`;
    },
  },
  {
...
export function detectForensicFindings(lineItems, opts = {}) {
  ...
  const { billingPeriod = null, supplier = null, fakturadatum = null } = opts;
  const findings = [];
    ...
    for (const d of DETECTORS) {                 // prioritetsordning → första träffen vinner (dedup per rad)
      if (d.re.test(desc) && (!d.guard || d.guard(desc))) {  // guard-fall (t.ex. inom-plan) → faller vidare
        findings.push({
          ...
          negotiable: true, text: d.text(desc, amount, fakturadatum),
          ...(d.derive ? d.derive(desc, amount, fakturadatum) : {}),
        });
