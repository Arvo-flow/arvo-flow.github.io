...
// Används av driftvakten (scripts/verify-tele2-bredband.mjs). Ingen DOM, ingen auth.
 ...
 */
export async function fetchTele2Products(addressId) {
  const r = await getJson(`${BASE}/broadband/products?category=REGULAR&addressId=${addressId}&groupAgreement=false&infrastructure=${INFRA}&entryId=${ENTRY_ID}`);
...
export async function tele2BroadbandFor(query) {
  ...
  const products = await fetchTele2Products(a.id);
