...
// ── TEASERN BÄR NU ETT VERIFIERAT TAL (2026-08-12) ──────────────────────────────────────────
// Raden sa "avgör tusenlappar per anställd och år" — en hedge, och kortets svagaste mening
// direkt under dess starkaste (de sex bolagen med orgnr). Det är bakvänt: teasern ska vara som
// vassast INNAN kunden investerat något.
//
// Talet nedan är samma aritmetik som licensspannet på det bekräftade kortet, minus det enda vi
// inte vet före bolagsvalet: antalet anställda. Kvar blir spannet PER ANVÄNDARE, som är ett
// faktum om Microsofts prislista — inte ett påstående om kundens faktura. Räknas ur prisboken,
// aldrig hårdkodat: en prishöjning ska flytta meningen automatiskt (regel 1).
function m365Nivabrygga() {
  const tiers = Object.values(BRANCHINDEX['saas-productivity']?.licenseTierBenchmarks ?? {})
    .filter((t) => t && t.currency === 'SEK' && Number.isFinite(t.msrpAnnual) && t.msrpAnnual > 0);
  if (tiers.length < 2) return 'Vilken nivå ni betalar för syns bara på fakturan. Dela en, så läser vi exakt.';
  const priser = tiers.map((t) => t.msrpAnnual).sort((a, b) => a - b);
  const lagst = priser[0], hogst = priser.at(-1);
  const perAr = Math.round((hogst - lagst) * 12).toLocaleString('sv-SE');
  const kr = (n) => n.toFixed(2).replace('.', ',');
  return `Microsofts publika listpriser spänner ${kr(lagst)}–${kr(hogst)} kr per användare och månad `
    + `— ${perAr} kr per anställd och år mellan lägsta och högsta nivå. Vilken ni ligger på står bara på fakturan.`;
}
const M365_NIVABRYGGA = m365Nivabrygga();
// Datumet kommer ur prisboken, så en gammal verifiering syns i kundytan i stället för att döljas.
const M365_VERIFIERAT = (() => {
  const d = Object.values(BRANCHINDEX['saas-productivity']?.licenseTierBenchmarks ?? {})
    .map((t) => t?.lastVerified).filter(Boolean).sort();
  return d.at(-1) ?? null;
})();
...
export function buildRevealFindings({ domain, posture, domainReg, ct }, { now = new Date() } = {}) {
  ...
  if (posture?.mx === 'microsoft365') {
    ...
    f.push({
      ...
      source: `Ert e-postsystem dirigeras till Microsoft — öppet i ${domain}:s publika uppgifter · `
        + `nivåspannet: Microsofts publika listpriser${M365_VERIFIERAT ? `, verifierade ${M365_VERIFIERAT}` : ''}`,
    });
  ...
  if (spfSuppliers.length >= 2) {
    ...
    f.push({
      ...
      detail: `${spfSuppliers.length} avsändare har rätt att skicka mejl i ert namn — avläsbart utifrån, utan att ni delat något. Ni ser listan nu på sekunder; era leverantörer har kunnat se den hela tiden. Vilka av dem ni betalar för syns i era fakturor — dela fakturorna, så läser vi dem exakt.`,
  ...
  if (f.length === 0) {
    ...
    f.push({
      ...
      detail: 'Vi väger varje faktura mot verifierade publika listpriser. Dela en faktura så ser vi ert.',
...
export function buildCrossReading({ domainReg, facts }) {
  ...
  return {
    ...
    detail: `Infrastrukturen är ${startYear - dy} år äldre än den resa ni är på. Om avtalen från den tiden följt med ser vi först i fakturorna.`,
...
export async function revealFromDomain(input, { fast = false } = {}) {
  ...
  const samla = { kandidater: [], sidtraffar: 0 };
  ...
  return {
    ...
    identity: {
      ...
      // HETTE readCount OCH NÅDDE KORTET SOM "25 LÄSTA" (rättat 2026-08-12). Mätningen visade
      // att söksidan har ett tak på 25 — skanska, avida och westander gav alla exakt 25, medan
      // trivector gav 17. Talet var en sidstorlek som utgav sig för att vara vårt arbete.
      // Namnet säger nu vad det är, så att ingen framtida yta kan råka lova något med det.
      sidtraffar: samla.sidtraffar,
    },
  };
}
...
export async function revealForConfirmedOrgnr(input, orgnr) {
  ...
  return {
    ...
    identity: { confirmed: bare, confirmedName: facts.legalName, candidates: [], sidtraffar: 0, byHuman: true },
  };
}
