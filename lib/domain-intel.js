...
export async function revealFromDomain(input) {
  ...
  // Det VERIFIERADE publika listpriset (kr/anv/mån på årsavtal, källa microsoft.com) — aldrig p25
  // (benchmark-estimat) i den här raden (grundarlärdom 2026-07-01: fel etikett = regel 3-brott).
  const lt = BRANCHINDEX?.['saas-productivity']?.licenseTierBenchmarks?.['business-standard'];
  const listPrice = lt?.msrpAnnual > 0
    ? { monthlyPerUserAnnualCommit: lt.msrpAnnual, product: 'Microsoft 365 Business Standard', lastVerified: lt.lastVerified ?? null }
    : null;
  const businessFindings = buildBusinessFindings(bizFacts, { listPrice });
