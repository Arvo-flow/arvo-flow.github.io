...
export async function revealFromDomain(input, { fast = false } = {}) {
  ...
  if (!posture.exists && !bizFacts) {
    return { domain, findings: [], reason: 'no-such-domain',
      note: `Vi hittar ingen aktiv domän bakom ${domain} — kontrollera stavningen, eller testa med en faktura så läser vi de verkliga talen.` };
  }
