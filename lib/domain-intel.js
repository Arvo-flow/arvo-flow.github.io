...
export async function revealFromDomain(input) {
  ...
  // Microsofts VERIFIERADE Business-planer (kr/anv/mån på årsavtal, microsoft.com) — HELA spannet,
  // aldrig en antagen plan (grundarlärdom 2026-07-01: DNS bevisar plattform, inte plan; ett "från
  // Standard-priset" var inte ens ett sant golv eftersom Basic är billigare).
  const lt = BRANCHINDEX?.['saas-productivity']?.licenseTierBenchmarks ?? {};
  const tiers = [
    ['business-basic', 'Business Basic'], ['business-standard', 'Business Standard'], ['business-premium', 'Business Premium'],
  ].filter(([k]) => lt[k]?.msrpAnnual > 0).map(([k, label]) => ({ label, monthly: lt[k].msrpAnnual }));
  const msTiers = tiers.length >= 2 ? { tiers, lastVerified: lt['business-standard']?.lastVerified ?? null } : null;
  const businessFindings = buildBusinessFindings(bizFacts, { msTiers });
