...
import { swMonthYear, MX_LABELS } from './format.js';
...
// ── Avslöjandet ────────────────────────────────────────────────────────────────
// Privata mejldomäner → inget bolagsavslöjande (vi har inget att läsa av).
...
export function buildRevealFindings({ domain, posture, domainReg, ct }, { now = new Date() } = {}) {
  ...
  } else if (posture?.mx && MX_LABELS[posture.mx] && posture.mx !== 'unknown') {
    f.push({
      kind: 'platform', confidence: 'high', title: `E-post via ${MX_LABELS[posture.mx]}`,
      detail: 'Avläst ur er e-poststack.', source: `MX-posten för ${domain}`,
    });
  }
