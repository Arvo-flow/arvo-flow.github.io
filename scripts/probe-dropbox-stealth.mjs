...
const usdHits = (t) => [...new Set((t.match(/.{0,18}(?:US)?\$\s?\d[\d.,]*.{0,8}/gi) || []).map((s) => s.replace(/\s+/g, ' ').trim()))].slice(0, 12);
function tierCtx(t) {
  const out = [];
...
for (const url of URLS) {
  ...
  if (sek.length) anySek = true; if (eur.length) anyEur = true;
...
await browser.close();
console.log(`\n[dropbox-stealth] klar · ÄKTA SEK hittat: ${anySek} · EUR (FX-gräns) hittat: ${anyEur}`);
console.log(anySek
  ? '→ KANDIDAT: Dropbox visar SEK publikt. Verifiera tier-struktur ovan + moms, stagea, bygg verifierare.'
  : anyEur
    ? '→ UNDERKÄNT (Zero Trust): Dropbox prissätter Sverige i EUR → kräver FX. Vi konverterar aldrig. Fraktjakt-disciplin.'
