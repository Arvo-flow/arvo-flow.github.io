// scripts/mock-landing-salj-v2.mjs — DESIGNMOCKUP v2: ÄKTA premium. Lärdomar ur v1-kritiken:
...
<!-- ═══ RIDÅN — mörkret börjar med en keyline och ett andetag ═══ -->
<div style="background:${D.bg};padding:0 26px;position:relative;overflow:hidden">
  ...
  <div style="max-width:820px;margin:0 auto;position:relative;padding:84px 0 100px">
...
<!-- ═══ 03 · ÅTER I LJUSET — priset som en mening ═══ -->
<div style="max-width:820px;margin:0 auto;padding:88px 26px 100px;text-align:center">
  <div style="font-family:${MONO};font-size:10px;letter-spacing:.3em;text-transform:uppercase;color:${L.brand};border-bottom:1px solid ${L.hair};padding-bottom:16px;text-align:left">03 · Priset</div>
  <div style="font-family:${SERIF};font-size:34px;font-weight:500;color:${L.ink};margin-top:52px;line-height:1.3">
  ...
  <p style="font-size:13.5px;color:${L.muted};max-width:460px;margin:20px auto 0;line-height:1.7">
    Vi tar aldrig ersättning från någon leverantör. Vi sitter på er sida av bordet — det är hela affärsidén.
  </p>
  <div style="font-family:${SERIF};font-style:italic;font-size:13px;color:${L.muted};margin-top:72px">Finansiell intelligens som aldrig sover.</div>
</div>
</body>`;

mkdirSync('/tmp/landing-salj-v2', { recursive: true });
...
for (const [w, tag] of [[390, 'mobil'], [1600, 'desktop']]) {
  ...
  await p.screenshot({ path: `/tmp/landing-salj-v2/${tag}.png`, fullPage: true });
  console.log('✓', tag);
}
await b.close();
