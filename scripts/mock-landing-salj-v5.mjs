// scripts/mock-landing-salj-v5.mjs — DESIGNMOCKUP v5: syntesen efter Gemini-rundan. Hero-CTA:n ÄR dörren ('Se ert bolag som marknaden ser det →'), fakturan sekundär; i dossiern kommer Avslöjandet FÖRST (01) och rummet efter (02). Ordningen vunnen: mejl före faktura, faktura efter bevis.. Lärdomar ur v1-kritiken:
...
<div style="max-width:920px;margin:0 auto;padding:110px 26px 96px;text-align:center">
  ...
  <div style="margin-top:44px">
    ...
    <div style="font-size:12.5px;color:${L.muted};margin-top:16px">två sekunder · öppna källor &nbsp;·&nbsp; <span style="text-decoration:underline;text-underline-offset:3px">eller testa med en faktura</span></div>
  </div>
...
<div style="max-width:1120px;margin:0 auto;padding:0 22px"><div style="background:${D.bg};border-radius:30px;padding:0 34px;position:relative;overflow:hidden;box-shadow:0 60px 140px rgba(8,15,13,.38)">
  ...
  <div style="max-width:760px;margin:0 auto;position:relative;padding:76px 0 92px">
    ...
    <div style="display:flex;justify-content:space-between;align-items:baseline;border-bottom:1px solid ${D.hair};padding-bottom:16px;margin-top:0">
      <span style="font-family:${MONO};font-size:10px;letter-spacing:.3em;text-transform:uppercase;color:${D.teal}">01 · Avslöjandet</span>
    ...
    <div style="display:flex;justify-content:space-between;align-items:baseline;border-bottom:1px solid ${D.hair};padding-bottom:16px;margin-top:110px">
      <span style="font-family:${MONO};font-size:10px;letter-spacing:.3em;text-transform:uppercase;color:${D.teal}">02 · Arvo-kontoret</span>
    ...
    <h2 style="font-family:${SERIF};font-size:46px;font-weight:500;margin:48px 0 0;line-height:1.15;${METALLIC}">
...
mkdirSync('/tmp/landing-salj-v5', { recursive: true });
...
for (const [w, tag] of [[390, 'mobil'], [1600, 'desktop']]) {
  ...
  await p.screenshot({ path: `/tmp/landing-salj-v5/${tag}.png`, fullPage: true });
  console.log('✓', tag);
}
await b.close();
